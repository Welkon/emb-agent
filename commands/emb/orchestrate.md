...
- In Pi, prefer the native `emb_subagent` tool. Active task implementation defaults to `fw-doer` followed by `release-checker`; add `hw-scout`, `bug-hunter`, `arch-reviewer`, or `sys-reviewer` only when their evidence is needed.
