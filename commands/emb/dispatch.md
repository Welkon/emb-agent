...
2. 读取返回的：
   ...
   - `dispatch_contract`
   - `context_hygiene`
3. 如果 `resolved_action = scan` 且 `tool_execution.available = true`：
   - 先看 `tool_execution.status`
   - 若为 `ready`，优先执行 `tool_execution.cli`
   - 若不是 `ready`，先根据 `missing_inputs` 或 adapter 缺口补齐，不要硬跑
   - 工具执行用于先收敛寄存器 / 公式 / 外设真值，不替代后续 scan 结论整合
...
5. 如果 `agent_execution.recommended = false`：
   - 直接 inline 执行对应动作
6. 子 agent 返回后，必须由当前线程整合成 emb 标准输出，不直接拼接原文。

## 要求

- 这是轻量分发，不引入阶段级厚 orchestration
- 不要同时让多个可写 agent 修改同一组文件
- clear context 风险以 `context_hygiene` 为准；必要时先 `pause -> clear -> resume`
- `dispatch next` 优先用于“我现在该怎么分发”的场景
- `dispatch next` 如果已经给出 `tool_execution`，说明 scan 前面还插了一步“先跑工具”
