...
- When `start` routes to `task add`, treat that as task intake: create the task/PRD first, then use `scan` if scope is still fuzzy or `plan` when the path is already explicit.
