...
- When you want to snapshot the current draft into a private catalog or private repository worktree, use:
  `adapter export [<source>] --chip <slug>`
- When a project-local draft reaches `reusable-candidate` and is ready for shared reuse, use:
  `adapter publish [<source>] --chip <slug>`
- `adapter analysis init` creates a schema-backed draft file under `.emb-agent/analysis/` so a local agent can keep filling it in safely.
