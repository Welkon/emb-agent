...
- `adapter derive` 也会为每个 tool 生成 `emb-agent/adapters/routes/<tool>.cjs` draft route；该 route 只暴露 binding 草稿，不执行真实公式
- `timer-calc` 例外：生成的 draft route 已带首版通用搜索实现，只要 binding 参数足够即可直接运行
