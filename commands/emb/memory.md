...
`mem` reads existing local Claude Code / Codex / Pi JSONL sessions and never uploads content. Search/context/show/related use `.emb-agent/cache/mem/index.json` and rebuild it when stale. The semantic-hash vector is deterministic/local; no embedding API or network call is used.
