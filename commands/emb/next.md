...
- If `next` returns `agent_protocol.gate.kind=prd-exploration`, follow `document_evidence_policy`: scan and ingest listed hardware evidence first when `hardware_first=true`, then ask detailed requirement/behavior questions, update `docs/prd/system.md` and `.emb-agent/req.yaml`, run the installed runtime's `validate` or `health` command, then stop until explicit agreement. Child execution PRDs are created in the later `prd-breakdown` gate, not during exploration.
