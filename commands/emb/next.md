...
- If `next` returns `agent_protocol.gate.kind=prd-exploration`, ask detailed requirement/behavior questions, update `docs/prd/system.md` and `.emb-agent/req.yaml`, run the installed runtime's `validate` or `health` command, then stop until explicit agreement. Child execution PRDs are created in the later `prd-breakdown` gate, not during exploration.
