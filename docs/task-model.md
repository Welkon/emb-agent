...
`task activate` and `task resolve` still create and cleanup the task workspace automatically, but the worktree lifecycle can now be managed independently when you need isolated inspection or pre-provisioning.
