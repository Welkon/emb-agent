...
```text
ingest doc -> review/apply -> adapter analysis init -> agent fills analysis artifact -> adapter derive --from-analysis -> next
```
...
- `adapter analysis init` to create the structured chip-support analysis artifact
- `adapter derive --from-analysis` to let derive/generate own the final draft adapter output
