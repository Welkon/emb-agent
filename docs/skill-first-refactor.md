...
workspace runtime beta is useful here because it makes one architectural decision very explicit: route by intent first, then load the right skill or sub-agent with the right context. We should copy that decision, not the entire workspace runtime filesystem model.
...
- Do not clone workspace runtime task/spec layout mechanically.
...
workspace runtime beta wins because the right context is assembled before implementation. We should do the same, but using emb-agent primitives.
...
## What We Should Copy From workspace runtime Beta
...
- A repo-wide `.workspace runtime/` clone
