...
After `--confirm`, the result includes `knowledge graph refresh`, a register-focused `knowledge graph explain <register>`, and `knowledge graph query <register>` in `next_steps`.
