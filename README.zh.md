...
emb-agent 内置一组面向特定工作流的子代理，AI 助手可将任务委派给它们。每个代理职责单一、边界清晰。在支持 subagent 的 host 上，如果 SDK、工具链、API 或供应商示例证据不足，active task 实现默认先派 `researcher`，再派 `fw-doer`，最后派独立的 `release-checker`；父会话负责协调、综合结果和关闭任务。

| 代理 | 职责 |
|---|---|
| **onboard** | 项目初始化与迁移 — 空仓库时创建 `.emb-agent/`，或审计已有硬件文档并映射到 emb-agent 结构 |
| **hw-scout** | 硬件事实调查 — 定位数据手册、原理图、引脚映射和寄存器级信息 |
| **researcher** | 代码、SDK、API、工具链、供应商示例和迁移调研，结果持久化到 task `research/` 文件 |
...
| **release-checker** | 独立实现检查、明确小问题自修、发布验证 |
...
| [命令参考](commands/emb/help.md) | 默认命令流 + 完整已安装命令文档；host 斜杠命令只暴露 `/emb-start`、`/emb-next`、`/emb-finish-work` |
