...
const schematicAdvisor = require(
	path.join(ROOT, "lib", "schematic-advisor.cjs"),
);
...
function parseArgs(argv) {
	...
	if (
		...
		throw new Error(
			"format must be auto, altium-json, altium-raw, netlist, bom-csv, or text",
		);
	}
...
function normalizeSchematicResult(summary) {
	...
	return {
		...
		apply_ready: Object.hasOwn(summary, "apply_ready")
			? summary.apply_ready
			: semantics.apply_ready,
	};
}
...
function collectObjects(node, results, depth) {
	...
	Object.values(node).forEach((value) =>
		collectObjects(value, results, depth + 1),
	);
}

function normalizeComponent(input) {
	const designator = ensureString(
		input.designator || input.refdes || input.ref,
	);
	...
	const component = {
		designator,
		value: ensureString(
			input.value || input.comment || input.description || "",
		),
		comment: ensureString(input.comment || input.description || ""),
		library_ref: ensureString(
			input.libref || input.library_ref || input.symbol || "",
		),
		libref: ensureString(
			input.libref || input.library_ref || input.symbol || "",
		),
		footprint: normalizePackage(
			input.footprint || input.package || input.pattern || "",
		),
		...
		mpn: ensureString(
			input.mpn || input.part_number || input.manufacturer_part_number || "",
		),
		parameters:
			input.parameters &&
...
function normalizeNet(input) {
	const hasGroupingFields =
		Object.hasOwn(input || {}, "members") ||
		Object.hasOwn(input || {}, "nodes") ||
		Object.hasOwn(input || {}, "connections") ||
		Object.hasOwn(input || {}, "pins");
	const hasNamedNetFields =
		Object.hasOwn(input || {}, "label") || Object.hasOwn(input || {}, "signal");
	const hasPinShape =
		Object.hasOwn(input || {}, "number") || Object.hasOwn(input || {}, "pin");
	const hasNetShape =
		hasGroupingFields || hasNamedNetFields || Object.hasOwn(input || {}, "net");
	if (hasPinShape && !hasGroupingFields && !hasNamedNetFields) {
		return null;
	}
	const name = ensureString(
		input.net || input.label || input.signal || (hasNetShape ? input.name : ""),
	);
	if (!name) {
		return null;
	}
	...
	return {
		...
		source_paths: makeArray(input.source_paths)
			.map(ensureString)
			.filter(Boolean),
...
function parseAltiumJson(text) {
	...
	const components = objects
		...
		.filter(
			(item, index, list) =>
				index ===
				list.findIndex((other) => other.designator === item.designator),
		);
	...
	const nets = objects
		...
		.filter(
			(item, index, list) =>
				index === list.findIndex((other) => other.name === item.name),
		);
...
function parseBomCsv(text) {
	...
	const designatorIndex = headers.findIndex((item) =>
		["designator", "designators", "refdes"].includes(item),
	);
	const valueIndex = headers.findIndex((item) =>
		["value", "comment", "description"].includes(item),
	);
	const footprintIndex = headers.findIndex((item) =>
		["footprint", "package", "pattern"].includes(item),
	);
...
function parseNetlistText(text) {
	...
	lines.forEach((line) => {
		...
		const componentMatch = trimmed.match(
			/\b([A-Za-z]{1,4}\d+)\b.*?\b(VS1838B?|HX1838|TSOP\d+|IRM\d+|MAX485|SP3485)\b/i,
		);
		if (componentMatch) {
		...
		const netMatch = trimmed.match(
			/\b([A-Za-z][A-Za-z0-9_/-]{1,63})\b\s*[:=-]?\s*(?:connects?|net)?/u,
		);
...
function annotateParsedSource(parsed, sourcePath, sourceIndex) {
	...
	next.nets = makeArray(next.nets).map((net) => {
		...
		return {
			...
			source_paths: runtime.unique([
				...makeArray(net.source_paths),
				sourcePath,
			]),
	...
	next.sheets =
		makeArray(next.sheets).length > 0
...
function mergeNets(nets) {
	...
	makeArray(nets).forEach((net) => {
		...
		current.members = runtime.unique(
			current.members.concat(
				makeArray(net.members).map(ensureString).filter(Boolean),
			),
		);
		current.evidence = runtime
			.unique(
				current.evidence.concat(
					makeArray(net.evidence).map((item) => JSON.stringify(item)),
				),
			)
			.map((item) => {
				try {
			...
			.filter(Boolean);
		current.confidence =
			current.confidence || ensureString(net.confidence || "");
		current.source_paths = runtime.unique(
			current.source_paths.concat(
				[ensureString(net.source_path)].filter(Boolean),
			),
		);
		current.sheets = runtime.unique(
			current.sheets.concat(
				makeArray(net.sheets).map(ensureString).filter(Boolean),
			),
		);
...
function isPowerNetName(name) {
	return /^(?:gnd|ground|agnd|dgnd|vss|vdd|vcc|vin|vbat|bat\+?|3v3|3\.3v|5v|12v|24v|\+?\d+(?:\.\d+)?v)$/i.test(
		String(name || "").trim(),
	);
}
...
function buildVisualNetlistAnalysis(sourcePaths, parsed) {
	...
	const namedNets = nets.filter(
		(net) =>
			ensureString(net.name) &&
			!isUnnamedNet(ensureString(net.name)) &&
			!ensureString(net.name).includes(":UNNAMED_NET_"),
	);
	const crossSheetNets = namedNets
		.filter(
			(net) =>
				makeArray(net.sheets).length > 1 ||
				makeArray(net.source_paths).length > 1,
		)
		.map((net) => ({
	...
	const signalCandidates = namedNets
		...
		.filter((net) =>
			/(?:ir|uart|rx|tx|pwm|key|sda|scl|spi|mosi|miso|clk|rst|reset|adc|dac|i2c|swclk|swdio)/i.test(
				net.name,
			),
		)
		.slice(0, 32)
	...
	return {
		...
		graph: {
			...
			unnamed_nets: nets.filter(
				(net) =>
					isUnnamedNet(net.name) ||
					ensureString(net.name).includes(":UNNAMED_NET_"),
			).length,
...
function buildHardwareDraft(sourcePath, parsed, mcuCandidates) {
	...
	const topCandidate =
		mcuCandidates && mcuCandidates.length > 0 ? mcuCandidates[0] : null;
	const truths = runtime.unique(
		[
			...
			visualNetlist.page_count > 1
				? `Multi-page schematic ingest: ${visualNetlist.page_count} pages, ${visualNetlist.graph ? visualNetlist.graph.cross_sheet_nets : 0} cross-sheet named nets`
				: "",
			namedNets.length > 0
				? `Named nets extracted: ${namedNets.join(", ")}`
				: "No named nets were extracted from the schematic input",
			topCandidate
				? `Top MCU candidate: ${topCandidate.designator} (${topCandidate.libref || topCandidate.value || "unknown model"}, ${topCandidate.footprint || "unknown package"}, score=${topCandidate.score})`
				: "",
		].filter(Boolean),
	);
	const unknowns = runtime.unique(
		[
			...
			"MCU symbol pin numbers are not GPIO names; package pin-to-GPIO/peripheral mapping requires datasheet/manual evidence or explicit user confirmation before hw.yaml/PRD promotion",
			"Named nets can suggest signal intent, but user-facing behavior and firmware requirements still need requirement exploration before PRD confirmation",
			components.length > 0
				? ""
				: "No components were normalized from the schematic input",
			nets.length > 0 ? "" : "No nets were normalized from the schematic input",
			!topCandidate
				? "No MCU candidate could be identified from the schematic components"
				: "",
		].filter(Boolean),
	);
	...
	return {
		mcu: {
			...
			model: topCandidate
				? topCandidate.libref || topCandidate.value || ""
				: "",
...
const MCU_KEYWORD_PATTERN =
	/\b(?:MCU|microcontroller|SoC|processor|controller|wireless\s*MCU|BLE\s*SoC)\b/i;
...
function identifyMcuCandidates(components) {
	...
	for (const component of components) {
		...
		if (
			PASSIVE_DESIGNATOR_RE.test(designator) ||
...
function buildAgentAnalysisHandoff(
	sourcePath,
	parsed,
	artifacts,
	mcuCandidates,
	...
	const visualNetlist =
		parsed && parsed.visual_netlist ? parsed.visual_netlist : {};
	...
	return {
		...
		evidence: {
			...
			components_with_package: components.filter((item) =>
				ensureString(item.package || item.footprint),
			).length,
			components_with_datasheet: components.filter((item) =>
				ensureString(item.datasheet),
			).length,
		...
		expected_output: [
			...
			"Do not translate symbol pin numbers into GPIO/peripheral roles until the exact package pin map is reviewed or the user confirms it.",
...
function buildHardwareReviewHandoff(parsed, artifacts) {
	const advice =
		parsed && parsed.schematic_advice ? parsed.schematic_advice : {};
	...
	return {
		...
		command:
			artifacts && artifacts.parsed
				? `schematic advice --parsed ${artifacts.parsed}`
				: "schematic advice",
		artifact:
			artifacts && artifacts.schematic_advice ? artifacts.schematic_advice : "",
		findings,
		warnings,
		errors,
		reminder_policy:
...
function schematicCacheIsComplete(artifactPaths) {
	...
	if (
		artifacts.preview_input &&
		!fs.existsSync(artifactPaths.previewInputJson)
	) {
		return false;
	}
	return true;
...
function confirmMcuToHardware(projectRoot, candidate, schematicPath, force) {
	...
	if (!fs.existsSync(hwPath)) {
		...
		if (!fs.existsSync(templatePath)) {
			throw new Error(
				"hw.yaml template not found; cannot create .emb-agent/hw.yaml",
			);
		}
	...
	content = attachProject.replaceScalarLine(
		content,
		"  vendor: ",
		candidate.vendor_guess || "",
		force,
	);
	content = attachProject.replaceScalarLine(
		content,
		"  model: ",
		candidate.libref || candidate.value || "",
		force,
	);
	content = attachProject.replaceScalarLine(
		content,
	...
	const schematicSources = readObjectListForSources(
		content,
		"  schematic:",
		"    ",
	);
	...
	if (!schematicSources.includes(schematicRelPath)) {
		...
		content = replaceObjectListBlockForSources(
			content,
			"  schematic:",
			"    ",
			schematicSources,
...
function readObjectListForSources(content, keyLine, listIndent) {
	...
	const start = lines.findIndex(
		(line) => line.trimStart() === keyLine.trimStart(),
	);
	if (start === -1) {
		return [];
	}
...
function replaceObjectListBlockForSources(
	content,
	keyLine,
	listIndent,
	values,
...
function ingestSchematic(argv, options) {
	...
	const projectRoot = path.resolve(
		args.project || (options && options.projectRoot) || process.cwd(),
	);
	...
	if (
		!args.force &&
		args.confirmMcu < 0 &&
		schematicCacheIsComplete(artifactPaths)
	) {
		...
		return {
			...
			last_files: [
				path
					.relative(projectRoot, artifactPaths.summaryJson)
					.replace(/\\/g, "/"),
			],
		};
	}
	...
	const parsedSources = sourceInputs.map((input, index) => {
		const parsedSource =
			input.detected_format === "altium-json"
	...
	const hardwareDraft = buildHardwareDraft(
		relativePaths,
		parsed,
		mcuCandidates,
	);
	...
	const componentRefs = [];
	const signalCandidates = makeArray(
		parsed.visual_netlist && parsed.visual_netlist.signal_candidates,
	);
	const hasPreview = Boolean(
		parsed.preview && parsed.preview.input && parsed.preview.svg,
	);
	const artifacts = {
		parsed: path
			.relative(projectRoot, artifactPaths.parsedJson)
			.replace(/\\/g, "/"),
		visual_netlist: path
			.relative(projectRoot, artifactPaths.visualNetlistJson)
			.replace(/\\/g, "/"),
		schematic_advice: path
			.relative(projectRoot, artifactPaths.schematicAdviceJson)
			.replace(/\\/g, "/"),
		summary: path
			.relative(projectRoot, artifactPaths.summaryJson)
			.replace(/\\/g, "/"),
		hardware_facts: path
			.relative(projectRoot, artifactPaths.hardwareYaml)
			.replace(/\\/g, "/"),
		hardware_facts_json: path
			.relative(projectRoot, artifactPaths.hardwareJson)
			.replace(/\\/g, "/"),
		source: path
			.relative(projectRoot, artifactPaths.sourceJson)
			.replace(/\\/g, "/"),
	};
	if (hasPreview) {
		artifacts.preview_input = path
			.relative(projectRoot, artifactPaths.previewInputJson)
			.replace(/\\/g, "/");
		artifacts.preview_svg = path
			.relative(projectRoot, artifactPaths.previewSvg)
			.replace(/\\/g, "/");
	}
	const topCandidateLabel =
		mcuCandidates.length > 0
	...
	const summary = {
		...
		parser: {
			...
			summary:
				"Schematic input was normalized into reusable board facts and visual-netlist analysis; all inferred pins and roles still need datasheet/manual confirmation",
		},
		summary: {
			components: (parsed.components || []).length,
			nets: (parsed.nets || []).length,
			pages: parsed.visual_netlist.page_count || relativePaths.length,
			cross_sheet_nets: parsed.visual_netlist.graph
				? parsed.visual_netlist.graph.cross_sheet_nets
				: 0,
			dangling_nets: parsed.visual_netlist.graph
				? parsed.visual_netlist.graph.dangling_nets
				: 0,
			preview: hasPreview ? "svg" : "",
			advice_findings:
				parsed.schematic_advice && parsed.schematic_advice.summary
					? parsed.schematic_advice.summary.findings
					: 0,
			advice_warnings:
				parsed.schematic_advice && parsed.schematic_advice.summary
					? parsed.schematic_advice.summary.warnings
					: 0,
	...
	summary.agent_analysis = buildAgentAnalysisHandoff(
		relativePaths.join(", "),
		parsed,
		summary.artifacts,
		mcuCandidates,
