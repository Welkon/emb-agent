...
const defaultWorkflowSourceHelpers = require(
	path.join(ROOT, "lib", "default-workflow-source.cjs"),
);
const workflowImportHelpers = require(
	path.join(ROOT, "lib", "workflow-import.cjs"),
);
const workflowRegistry = require(
	path.join(ROOT, "lib", "workflow-registry.cjs"),
);
...
function writePromptOutput(hostProcess, text) {
	const output =
		hostProcess && hostProcess.stdout ? hostProcess.stdout : process.stdout;
...
function buildWorkflowSpecPrompt(entries, options = {}) {
	...
	return [
		...
		chalk.gray(
			"  Press Enter to continue without extra specs, or use comma-separated numbers for multiple specs.",
		),
		"",
...
function promptWorkflowSpecSelection(entries, options = {}) {
	...
	if (typeof options.promptWorkflowSpecChoices === "function") {
		...
		if (Array.isArray(prompted)) {
			return runtime.unique(
				prompted.map((item) => String(item || "").trim()).filter(Boolean),
			);
		}
	...
	while (true) {
		writePromptOutput(
			hostProcess,
			`${buildWorkflowSpecPrompt(entries, options)}\n`,
		);
		try {
			return parseWorkflowSpecSelection(
				readInteractiveLineSync(hostProcess),
				entries,
			);
		} catch (error) {
			const chalk = createPromptStyler(hostProcess);
...
function parseArgs(argv) {
	...
	function setRuntime(value, token) {
		...
		if (result.runtimeSet && result.runtime !== normalized) {
			throw new Error(
				`Conflicting runtime options: ${result.runtime} vs ${normalized}`,
			);
		}
...
function expandWorkspacePattern(projectRoot, pattern) {
	...
	return fs
		...
		.map((entry) =>
			normalizeMonorepoPath(path.posix.join(baseDir, entry.name)),
		);
}
...
function detectMonorepoPackages(projectRoot) {
	...
	const packages = Array.from(byPath.values()).sort((left, right) =>
		left.path.localeCompare(right.path),
	);
	const defaultPackage =
		packages.find((item) => item.submodule !== true) || packages[0] || null;
...
function shouldAttemptDefaultWorkflowImport(
	initOptions,
	options,
	workflowCatalog,
	explicitSpecs,
	...
	if (
		findMissingExplicitWorkflowSpecs(workflowCatalog, explicitSpecs).length > 0
	) {
		return true;
	}

	const selectableSpecs = (workflowCatalog.specs || []).filter(
	...
	return (
		typeof options.promptWorkflowSpecChoices === "function" ||
...
function prepareProjectWorkflowSetup(projectRoot, args, options = {}) {
	...
	let workflowRegistryImport = initOptions.registry
		? workflowImport.importProjectWorkflowRegistry(
				projectRoot,
				initOptions.registry,
				{
		...
		: null;
	const explicitSpecs = runtime.unique(
		(initOptions.specs || []).filter(Boolean),
	);
	let workflowCatalog = loadWorkflowCatalog(projectRoot);
	const missingExplicitSpecs = findMissingExplicitWorkflowSpecs(
		workflowCatalog,
		explicitSpecs,
	);

	if (
		shouldAttemptDefaultWorkflowImport(
			initOptions,
			options,
			workflowCatalog,
			explicitSpecs,
		...
		const defaultWorkflowSource =
			defaultWorkflowSourceHelpers.resolveDefaultWorkflowSource(
				RUNTIME_CONFIG,
				process.env,
			);
	...
	const remainingMissingSpecs = findMissingExplicitWorkflowSpecs(
		workflowCatalog,
		explicitSpecs,
	);
	if (remainingMissingSpecs.length > 0) {
		...
		throw new Error(
			`Workflow ${label} not found: ${remainingMissingSpecs.join(", ")}`,
		);
	}
...
function buildTemplateIndex(registry) {
	const templates =
		registry && Array.isArray(registry.templates) ? registry.templates : [];
	const byName = {};
	const byOutput = {};
...
function buildTemplateContext(projectRoot, projectConfig) {
	const developer = runtime.validateDeveloperConfig(
	...
	return {
		...
		REQ_CONSTRAINT_1:
			"Prefer reusing the existing codebase and hardware truth before expanding architecture",
		ACCEPTANCE_1:
			"The current goal can be confirmed at board level or through a minimal verification path",
		FAILURE_POLICY_1:
			"When hardware or requirements are unconfirmed, record an unknown first instead of guessing",
		REQ_UNKNOWN_1:
...
function createTemplateFile(
	...
	const content = applyTemplate(runtime.readText(meta.absolute_path), context);
...
function buildProjectConfig(projectRoot, args, options = {}) {
	...
	const workflowCatalog =
		options.workflowCatalog || loadWorkflowCatalog(projectRoot);
	const monorepo = detectMonorepoPackages(projectRoot);
	const active_specs = runtime.unique(
		(Array.isArray(options.activeSpecs)
			? options.activeSpecs
			: args.specs || []
		).filter(Boolean),
	);

	active_specs.forEach((name) =>
		ensureSelectableSpecExists(projectRoot, name, workflowCatalog),
	);
	...
	return runtime.validateProjectConfig(
		{
			...
			developer: {
				name:
					args.user ||
					(RUNTIME_CONFIG.developer && RUNTIME_CONFIG.developer.name) ||
					"",
				runtime:
					args.runtime ||
					(RUNTIME_CONFIG.developer && RUNTIME_CONFIG.developer.runtime) ||
					"",
			},
...
function buildDocsPlan(projectRoot, projectConfig, registry) {
	...
	const selectedSpecs = projectConfig.active_specs.map((name) =>
		loadSelectableSpecForProject(projectRoot, name, workflowCatalog),
	);
...
function buildTruthPlan() {
	return [
		{
			output: runtime.getProjectAssetRelativePath("hw.yaml"),
			template: "hw-truth",
		},
		{
			output: runtime.getProjectAssetRelativePath("req.yaml"),
			template: "req-truth",
		},
	];
}
...
function ensureSystemPrd(projectRoot, projectConfig, force, options = {}) {
	const settings =
		options && typeof options === "object" ? options : { language: options };
...
function ensureProjectAgentsGuide(projectRoot, force, language, runtimeTarget) {
	...
	if (isClaude) {
		...
		const claudeExisting = claudeExisted
...
function buildBootstrapDocsPlan(projectRoot, projectConfig, registry) {
	return [
		{
			output: path.join("docs", "MCU-FOUNDATION-CHECKLIST.md"),
			template: "mcu-foundation-checklist",
		},
...
function ensureBootstrapTask(projectRoot, projectConfig, docsPlan, force) {
	...
	const manifest = {
		...
		description:
			"Review minimum project truth and decide which note templates should be created next.",
		status: "planning",
		dev_type: "docs",
		scope: "bootstrap",
		priority: "P1",
		...
		context: Object.fromEntries(
			BOOTSTRAP_TASK_CHANNELS.map((channel) => [
				channel,
				path
					.relative(projectRoot, path.join(taskDir, `${channel}.jsonl`))
					.replace(/\\/g, "/"),
			]),
		),
		created_at: now,
...
function scaffoldProject(projectRoot, projectConfig, force, options) {
	...
	const shouldUpdateDeveloper = Boolean(
		initOptions.userSet || initOptions.runtimeSet,
	);
	...
	if (!fs.existsSync(projectConfigPath) || force) {
		...
		if (shouldUpdateDeveloper) {
			const existing = runtime.validateProjectConfig(
				runtime.readJson(projectConfigPath),
				RUNTIME_CONFIG,
			);
			const nextDeveloper = runtime.validateDeveloperConfig({
				...(existing.developer || {}),
				...(initOptions.userSet ? { name: initOptions.user || "" } : {}),
				...(initOptions.runtimeSet
					? { runtime: initOptions.runtime || "" }
					: {}),
			});
			...
			fs.writeFileSync(
				projectConfigPath,
				JSON.stringify(effectiveProjectConfig, null, 2) + "\n",
				"utf8",
			);
	...
	const shouldWriteDeveloperMarker =
		force || shouldUpdateDeveloper || !developerExisted;
	...
	ensureGitignoreRule(
		projectRoot,
		runtime.getProjectAssetRelativePath(".developer"),
	);
	ensureGitignoreRule(
		projectRoot,
		`${runtime.getProjectAssetRelativePath("worktrees")}/`,
	);
	...
	const bootstrapDocsPlan = buildBootstrapDocsPlan(
		projectRoot,
		effectiveProjectConfig,
		workflowCatalog,
	);
	const systemPrdResult = ensureSystemPrd(
		projectRoot,
		effectiveProjectConfig,
		force,
		{
	...
	const projectAgentsGuide = ensureProjectAgentsGuide(
		projectRoot,
		force,
	...
	const bootstrapTask = ensureBootstrapTask(
		projectRoot,
		effectiveProjectConfig,
		bootstrapDocsPlan,
		force,
...
function buildLazyGenerationSummary() {
	return {
		...
		summary:
			"Init creates the minimum project skeleton; chip/tool support and capability assets are generated only when a concrete run needs them.",
	};
}
