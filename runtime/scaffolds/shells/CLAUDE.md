...
Use `help` for the default command flow. All installed command docs remain available under the host runtime (for example `.claude/emb-agent/commands/emb/`); prefer `onboard` and `next`, but use specialized commands when runtime output or the user request calls for them.
