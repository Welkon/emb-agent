...
| SDK/toolchain/API/vendor research for a task | Active task PRD; `.emb-agent/tasks/<task>/research/`; relevant local docs or fetched sources | Delegate `researcher` when available and persist reusable findings to `.emb-agent/tasks/<task>/research/<topic>.md` |
...
| Knowledge capture after work | `.emb-agent/workflow.md` | Record repeatable, expensive, or non-obvious lessons only when a durable lesson actually emerged |
