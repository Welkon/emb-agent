...
async function main(argv) {
  ...
  process.stderr.write('emb-agent: Rust binary not found, falling back to Node runtime\n');
...
function loadMainModule() {
  if (!_mainModule) _mainModule = require(path.join(ROOT, 'lib', 'emb-agent-main.cjs'));
  return _mainModule;
}
...
const exported = new Proxy(
  { main, loadMainModule },
  {
    ...
    has(target, prop) { return Reflect.has(target, prop) || prop in loadMainModule(); },
    ownKeys(target) { return Array.from(new Set([...Reflect.ownKeys(target), ...Reflect.ownKeys(loadMainModule())])); },
