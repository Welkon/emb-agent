...
function createWorkflowAuthoringHelpers(deps) {
  ...
  function createProjectSpec(options) {
    ...
    const name = slugify(config.name);
    const force = config.force === true;
    const dependsOnSpec = config.spec ? slugify(config.spec) : '';
    ...
    const selectable = !dependsOnSpec && !always;
    const autoInject = selectable || Boolean(dependsOnSpec || always);
    const applyWhen = selectable
      ? { specs: [name] }
      : dependsOnSpec
        ? { specs: [dependsOnSpec] }
        : always
    ...
    registry.specs = upsertNamedEntry(registry.specs, {
      ...
      selectable,
      priority: selectable ? 65 : dependsOnSpec ? 60 : always ? 55 : 50,
      apply_when: applyWhen,
      focus_areas: [],
      extra_review_axes: [],
      preferred_notes: withTemplate ? [templateOutput] : [],
      default_agents: []
    });
    saveProjectRegistry(paths, registry);
  ...
  function parseSpecArgs(args) {
    ...
    for (let index = 0; index < args.length; index += 1) {
      ...
      if (token === '--spec') {
        ...
        if (!nextToken) {
          throw new Error('Missing parent spec name for spec');
        }
        options.spec = nextToken;
        index += 1;
        continue;
      }
      if (token === '--always') {
    ...
    if (options.spec && options.always) {
      throw new Error('Use either --spec or --always for workflow spec injection');
    }
  ...
  function listWorkflowCatalog() {
    ...
    return {
      ...
      specs: (registry.specs || []).map(item => ({
        ...
        preferred_notes: item.preferred_notes || []
      })),
