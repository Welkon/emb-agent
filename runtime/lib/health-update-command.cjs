...
const DEFAULT_ADAPTER_SOURCE_BOOTSTRAP_CLI = `${runtimeHostHelpers.buildCliCommand(RUNTIME_HOST, ["support", "bootstrap"])}`;
...
function createHealthUpdateCommandHelpers(deps) {
	...
	function getDefaultAdapterSource() {
		return defaultAdapterSourceHelpers.resolveDefaultAdapterSource(
			RUNTIME_CONFIG,
			process && process.env,
		);
	}
	...
	function buildSupportCli(args) {
		...
		return command.replace(
			" emb-agent.cjs adapter ",
			" emb-agent.cjs support ",
		);
	}
	...
	function buildDefaultAdapterSourceAddCommand() {
		...
		const sourceArgs =
			defaultAdapterSourceHelpers.buildDefaultAdapterSourceArgs(source);
	...
	function findPendingProjectInputIntake(projectRoot, hardwareIdentity) {
		const hardwareReady = Boolean(
			hardwareIdentity && hardwareIdentity.model && hardwareIdentity.package,
		);
		if (
			hardwareReady ||
			!attachProjectCli ||
			typeof attachProjectCli.detectProjectInputs !== "function"
		) {
			return null;
		}

		const intake = projectInputIntake.buildPendingProjectInputIntake(
			projectRoot,
			{
				fs,
				path,
		...
		return {
			...
			label:
				intake.preferred.type === "schematic"
	...
	function findProjectLocalChipSupport(model, packageName) {
		...
		const candidates = runtime.unique(
			[
				...
				normalizedPackage
					? compactHardwareSlug(`${normalizedModel}${normalizedPackage}`)
					: "",
				normalizedPackage
					? compactHardwareSlug(`${normalizedModel}-${normalizedPackage}`)
					: "",
	...
	function readHookVersion() {
		...
		const versionLine = lines.find((line) =>
			line.includes("emb-hook-version:"),
		);
		if (!versionLine) {
			return "";
		}
	...
	function loadHardwareIdentity() {
		const hwPath = runtime.resolveProjectDataPath(
			resolveProjectRoot(),
			"hw.yaml",
		);
	...
	function normalizeActiveSpecsForDoctor(projectConfig, session) {
		if (
			projectConfig &&
			Array.isArray(projectConfig.active_specs) &&
			projectConfig.active_specs.length > 0
		) {
			return runtime.unique(projectConfig.active_specs);
		}
		if (
			session &&
			Array.isArray(session.active_specs) &&
			session.active_specs.length > 0
		) {
	...
	function buildSpecsDoctorReport() {
		...
		const session =
			resolved && resolved.session ? resolved.session : rawSession;
		const activeSpecs = normalizeActiveSpecsForDoctor(projectConfig, session);
		const workflowPaths =
			workflowRegistry.getProjectWorkflowPaths(projectExtDir);
		const registry = workflowRegistry.loadWorkflowRegistry(rootDir, {
			projectExtDir,
		});
		const checks = [];
		const recommendations = [];
		const registryByName = new Map(
			(registry.specs || []).map((item) => [item.name, item]),
		);
		...
		const snapshot = workflowRegistry.buildInjectedSpecSnapshot(
			...
			{
				profile:
					resolved && resolved.profile && resolved.profile.name
						? resolved.profile.name
						: String(
								(session && session.project_profile) ||
									(projectConfig && projectConfig.project_profile) ||
									"",
							).trim(),
				specs: activeSpecs,
				active_package:
					session && session.active_package ? session.active_package : "",
				default_package:
					session && session.default_package ? session.default_package : "",
		...
		const selectedCodeWritingNames = selectedCodeWritingSpecs.map(
			(item) => item.name,
		);
		const specsDoctorSummary =
		...
		checks.push(
			createCheck(
				...
				[
					...
					fs.existsSync(workflowPaths.specsDir)
						? formatStateEvidencePath(projectRoot, workflowPaths.specsDir)
						: "",
				],
				fs.existsSync(workflowPaths.registryPath)
					? ""
					: "Run workflow init to normalize .emb-agent/registry and .emb-agent/specs.",
			),
		);
		checks.push(
			createCheck(
		...
		checks.push(
			createCheck(
				...
				unresolvedSpecs.length > 0
					? "Some active specs are not usable"
					: "All active specs are resolvable and selectable",
				unresolvedSpecs.length > 0
					? unresolvedSpecs
					: resolvedSpecs.map((item) => `${item.name} -> ${item.display_path}`),
				unresolvedSpecs.length > 0
					? "Fix active_specs or register the missing selectable specs."
					: "",
			),
		);
		checks.push(
			...
			createCheck(
				...
				selectedCodeWritingSpecs.length > 0
					? [
							...
							runtimeHostHelpers.buildCliCommand(RUNTIME_HOST, [
								"capability",
								"run",
								"do",
								"--brief",
		...
		if (activeSpecs.length === 0 && selectedCodeWritingSpecs.length === 0) {
			recommendations.push(
				"Activate the project specs that should steer AI behavior, or check that built-in baseline specs are installed.",
			);
		}
		if (unresolvedSpecs.length > 0) {
			recommendations.push(
				"Fix .emb-agent/project.json active_specs or the workflow registry before relying on spec enforcement.",
			);
		}
		if (selectedCodeWritingSpecs.length === 0) {
			recommendations.push(
				"To force source-edit rules, install built-in baseline specs or activate a code-writing vendor/domain spec, then rerun health specs.",
			);
		}
		if (selectedCodeWritingSpecs.length > 0) {
			recommendations.push(
				"Before source edits, run capability run do or task activate so required_code_writing_specs is visible in the action context.",
			);
		}
		...
		return {
			...
			runtime_host:
				runtimeHostHelpers.resolveRuntimeHostFromModuleDir(__dirname).name,
			...
			specs_doctor: {
				...
				registry_paths:
					snapshot.registry_paths || registry.registry_paths || {},
	...
	function createBootstrapStage(id, status, label, options) {
		...
		return {
			...
			additional_steps: Array.isArray(config.additional_steps)
				? config.additional_steps.filter(Boolean)
				: [],
			manual: status === "manual",
			blocking: ["ready", "manual"].includes(status),
			evidence: Array.isArray(config.evidence)
				? config.evidence.filter(Boolean)
				: [],
		};
	}
	...
	function getQuickstartFollowup(nextStage, quickstartStage) {
		...
		if (
			quickstartStage === "restart-host-hooks" ||
		...
		if (
			quickstartStage === "fill-hardware-identity" ||
	...
	function buildChipSupportReuseSummary(adapterReusability) {
		const reusability =
			adapterReusability &&
			typeof adapterReusability === "object" &&
			!Array.isArray(adapterReusability)
				? adapterReusability
				: null;
	...
	function buildActionCardFromBootstrap(bootstrap) {
		...
		const quickstart =
			plan.quickstart && typeof plan.quickstart === "object"
				? plan.quickstart
				: {};
		const nextStage =
			plan.next_stage && typeof plan.next_stage === "object"
				? plan.next_stage
				: {};
		const steps = Array.isArray(quickstart.steps)
			? quickstart.steps.filter(Boolean)
			: [];
		...
		return {
			...
			stage:
				plan.display_current_stage ||
				getBootstrapStageDisplayId(plan.current_stage),
			action: nextStage.action_summary || getBootstrapStageAction(nextStage.id),
			summary:
				plan.display_summary || quickstart.user_summary || plan.summary || "",
			reason: nextStage.label || "",
			first_step_label: firstStep.label || "",
			first_instruction: firstStep.cli
				? ""
				: quickstart.user_summary ||
					nextStage.summary ||
					firstStep.label ||
	...
	function buildBootstrapPlan(
		...
		const findCommand = (...keys) =>
			commands.find((item) => keys.includes(item.key));
		const findCheck = (key) =>
			allChecks.find((item) => item.key === key) || null;
		const initReady = [
			"emb_agent_dir",
			"project_config_file",
		...
		const hardwareReady = Boolean(
			hardwareIdentity.model && hardwareIdentity.package,
		);
		const docApply =
			pendingDocApply && pendingDocApply.command
				? {
						...pendingDocApply,
						kind: "doc",
				...
				: null;
		const sourceIntake =
			pendingSourceIntake && pendingSourceIntake.cli
				? {
						...pendingSourceIntake,
						kind: "ingest",
		...
		const showAuthorizationStage = !(
			workspaceTrust &&
			workspaceTrust.trusted &&
			workspaceTrust.source === "host-config"
		);
				...
				createBootstrapStage(
					...
					"Enable host startup hooks",
					{
						...
						evidence: workspaceTrust
							? [
									workspaceTrust.source
										? `source=${workspaceTrust.source}`
										: "",
									workspaceTrust.signal
										? `signal=${workspaceTrust.signal}`
										: "",
								]
							: [],
					},
		...
		stages.push(
			createBootstrapStage(
				"hardware-truth",
				!initReady || !trustReady || sourceIntake
					? "pending"
					: hardwareReady
						? "completed"
						: "manual",
		...
		stages.push(
			createBootstrapStage(
				"doc-truth-sync",
				!initReady || !trustReady || sourceIntake || !hardwareReady
					? "pending"
					: docApply
						? "ready"
						: "completed",
		...
		if (bootstrap || analysisInit || derive) {
			...
			stages.push(
				createBootstrapStage(
					...
					bootstrap
						...
						: "Prepare project-local chip support",
					{
								...
								? [
										{
											label:
												derive.summary ||
												"Derive draft chip support into the current project",
											cli: derive.cli || "",
										},
									]
								: [],
			...
			stages.push(
				createBootstrapStage(
					...
					{
						summary:
							"Chip support registration and hardware matching are already closed",
					},
				),
			);
		}

		const hasBlockingStage = stages.some((item) =>
			["ready", "manual"].includes(item.status),
		);
		stages.push(
			createBootstrapStage(
				"next-step",
		...
		const quickstartSteps = [
			...(nextStage && nextStage.cli
				...
				: []),
			...(nextStage && Array.isArray(nextStage.additional_steps)
				? nextStage.additional_steps
				: []),
		...
		const quickstartFollowup = getQuickstartFollowup(
			nextStage,
			quickstartStage,
		);
		...
		const actionCard = buildActionCardFromBootstrap({
			status: nextStage
				? nextStage.status === "manual"
					? "manual"
					: "ready"
				: "complete",
			display_status: nextStage
				? getBootstrapStatusDisplay(nextStage.status)
				: "done",
			summary: nextStage
				? nextStage.id === "next-step" && reuseSummary
					? reuseSummary
			...
			display_summary: nextStage
				? nextStage.id === "next-step" && reuseSummary
					? reuseSummary
					: getQuickstartUserSummary(
							quickstartStage,
							nextStage.summary || nextStage.label,
						)
				: reuseSummary ||
					"Bootstrap is already clear. Enter the recommended next stage.",
			current_stage: nextStage ? nextStage.id : "",
			display_current_stage: nextStage
				? nextStage.display_id
				: "continue-with-next",
		...
		return {
			...
			quickstart:
				actionCard.first_cli || actionCard.then_cli || actionCard.followup
					? {
							stage: quickstartStage,
							display_stage: getQuickstartDisplayStage(quickstartStage),
							summary: quickstartSummary,
	...
	function findLatestHardwareDoc(projectRoot, pendingDocApply) {
		...
		const documents = Array.isArray(listing && listing.documents)
			? listing.documents
			: [];
		const blockedDocId =
			pendingDocApply && pendingDocApply.doc_id ? pendingDocApply.doc_id : "";
	...
	function buildDocAnalysisCommands(projectRoot, docEntry) {
		if (
			...
			!docEntry.doc_id ||
			!ingestDocCli ||
			typeof ingestDocCli.showDoc !== "function"
		) {
			return null;
		}
		...
		try {
			...
			const summaryInfo =
				view && view.summary_info && typeof view.summary_info === "object"
					? view.summary_info
					: null;
			const agentAnalysis =
				summaryInfo &&
				summaryInfo.agent_analysis &&
				typeof summaryInfo.agent_analysis === "object"
					? summaryInfo.agent_analysis
					: null;
			const recommendedFlow =
				summaryInfo &&
				summaryInfo.recommended_flow &&
				typeof summaryInfo.recommended_flow === "object"
					? summaryInfo.recommended_flow
					: null;
			const handoffProtocol =
				summaryInfo &&
				summaryInfo.handoff_protocol &&
				typeof summaryInfo.handoff_protocol === "object"
					? summaryInfo.handoff_protocol
					: null;
			...
			return {
				...
				init_argv: Array.isArray(agentAnalysis.init_argv)
					? agentAnalysis.init_argv
					: [],
				derive_command: agentAnalysis.derive_command || "",
				derive_argv: Array.isArray(agentAnalysis.derive_argv)
					? agentAnalysis.derive_argv
					: [],
	...
	function buildHealthReport() {
		const runtimeHost =
			typeof getRuntimeHost === "function" ? getRuntimeHost() : RUNTIME_HOST;
		const projectRoot = resolveProjectRoot();
		const projectExtDir = getProjectExtDir();
		...
		const adapterCacheDir = path.join(
			projectExtDir,
			"cache",
		...
		let rawSession = null;
		let handoff = null;
		const workspaceTrust = hookTrustHelpers.resolveWorkspaceTrust(
			null,
			process.env,
			{
				fs,
		...
		const subagentBridge =
			runtimeHost && runtimeHost.subagentBridge
		...
		checks.push(
			createCheck(
				...
				fs.existsSync(projectRoot)
					? ""
					: "Confirm first that the current cwd is the project root.",
			),
		);
		...
		checks.push(
			createCheck(
				...
				fs.existsSync(projectExtDir)
					? ".emb-agent directory exists"
					: ".emb-agent directory is missing",
				[
					path.relative(projectRoot, projectExtDir) ||
						runtime.getProjectAssetRelativePath(),
				],
				fs.existsSync(projectExtDir)
					? ""
					: "Run init first to generate the minimal .emb-agent project skeleton.",
			),
		);
		...
		checks.push(
			createCheck(
				...
				fs.existsSync(projectConfigPath)
					? ""
					: `Run init first to create ${runtime.getProjectAssetRelativePath("project.json")}.`,
			),
		);
			...
			checks.push(
				createCheck(
					...
					projectConfig
						...
						: "project.json is not initialized yet",
					projectConfig
						? [
					...
					projectConfig
						? ""
						: "Run init first to write the minimal project configuration.",
				),
			);
		} catch (error) {
			checks.push(
		...
		const requirementTruth = prdCommand.summarizeRequirementTruth(projectRoot, {
			fs,
			path,
			runtime,
		});
		checks.push(
			createCheck(
				"req_truth",
				requirementTruth.status === "ready"
					? "pass"
					: requirementTruth.status === "template"
						? "warn"
						: "fail",
				requirementTruth.status === "ready"
					? "req.yaml contains non-template requirement truth"
					: requirementTruth.status === "template"
						? "req.yaml still contains template requirement placeholders"
						: "req.yaml is missing",
				[
					runtime.getProjectAssetRelativePath("req.yaml"),
					...(requirementTruth.template_markers || []).map(
						(item) => `template_marker=${item}`,
					),
				],
				requirementTruth.recommendation ||
					`Complete ${runtime.getProjectAssetRelativePath("req.yaml")} first to record goals / features / acceptance.`,
			),
		);
		...
		[
			[
				...
				"fail",
				"hw.yaml is missing",
				`Complete ${runtime.getProjectAssetRelativePath("hw.yaml")} first to record MCU / pin / constraint ground truth.`,
			],
			[
				"docs_dir",
				docsDir,
				...
				"info",
				"docs directory has not been created yet",
				"Created on first saved document or manual docs work.",
			],
			[
				"doc_cache_dir",
				docCacheDir,
				"pass",
				"Document cache directory exists",
				"info",
				"Document cache directory has not been created yet",
				`Created on first document ingest under ${runtime.getProjectAssetRelativePath("cache", "docs")}.`,
			],
			[
				"chip_support_cache_dir",
				adapterCacheDir,
				"pass",
				"Chip support cache directory exists",
				"info",
				"Chip support cache directory has not been created yet",
				`Created on first support bootstrap/sync under ${runtime.getProjectAssetRelativePath("cache", "chip-support-sources")}.`,
			],
			[
				"chip_support_dir",
				adaptersDir,
				"pass",
				"Chip support directory exists",
				"info",
				"Chip support directory has not been created yet",
				`Created when project-local chip support is bootstrapped, synced, or derived under ${runtime.getProjectAssetRelativePath("chip-support")}.`,
			],
		].forEach(
			([
				key,
		...
		if (fs.existsSync(stateInspection.handoffPath)) {
			try {
				handoff = runtime.validateHandoff(
					runtime.readJson(stateInspection.handoffPath),
					RUNTIME_CONFIG,
				);
				checks.push(
					createCheck(
		...
		checks.push(
			createCheck(
				...
				unresolvedSpecs.length > 0 ? "fail" : "pass",
				unresolvedSpecs.length > 0
					? "There are unresolved workflow specs"
					: "Current workflow specs are resolvable",
				unresolvedSpecs.length > 0
					? unresolvedSpecs
					: [`specs=${desiredSpecs.join(",") || "(none)"}`],
				unresolvedSpecs.length > 0
					? "Fix the specs in project.json or add the matching workflow definition."
					: "",
			),
		);
		...
		if (!hardwareIdentity.model) {
			...
			const chipProfile = findChipProfileByModel(
				hardwareIdentity.model,
				hardwareIdentity.package,
			);
			const localChipSupport = findProjectLocalChipSupport(
				hardwareIdentity.model,
				hardwareIdentity.package,
			);
			checks.push(
				createCheck(
					...
					chipProfile || localChipSupport
						? [
								...
								localChipSupport
									? `project_chip_profile=${path.relative(projectRoot, localChipSupport.path).replace(/\\/g, "/")}`
									: "",
							]
						: [`model=${hardwareIdentity.model}`],
					chipProfile || localChipSupport
						? ""
						: "Tool auto-discovery can fully connect only after the adapter/chip profile is added.",
				),
			);
		}

		const pendingSourceIntake = findPendingProjectInputIntake(
			projectRoot,
			hardwareIdentity,
		);
		if (pendingSourceIntake) {
			checks.push(
				createCheck(
					...
					[pendingSourceIntake.file, `type=${pendingSourceIntake.type}`],
		...
		if (
			...
			projectConfig.integrations.mineru
		) {
			const mineru = projectConfig.integrations.mineru;
			const apiKeyConfigured =
				Boolean(mineru.api_key) ||
				Boolean(process.env[mineru.api_key_env || "MINERU_API_KEY"]);
			checks.push(
				createCheck(
					"mineru_integration",
		...
		if (projectConfig) {
			const latestHardwareDoc = findLatestHardwareDoc(
				projectRoot,
				pendingDocApply,
			);
			const localProjectSupport = hardwareIdentity.model
				? findProjectLocalChipSupport(
						hardwareIdentity.model,
						hardwareIdentity.package,
					)
				: null;
			const adapterSourceStatus = adapterSources.listSourceStatus(
				rootDir,
				projectRoot,
				projectConfig,
			);
			const enabledSources = adapterSourceStatus.filter(
				(item) => item.enabled !== false,
			);
			const syncedProjectSources = enabledSources.filter(
				(item) =>
					item.targets && item.targets.project && item.targets.project.synced,
			);
			const matchedProjectSources = syncedProjectSources.filter((item) => {
				const selection = item.targets.project.selection;
				return selection &&
					selection.filtered &&
					Array.isArray(selection.matched && selection.matched.chips)
			...
			checks.push(
				createCheck(
					...
					enabledSources.length > 0
						? "Chip support sources are registered"
						: "No chip support source is registered yet",
					enabledSources.length > 0
			...
			if (enabledSources.length === 0 && !localProjectSupport) {
				if (hardwareIdentity.model && latestHardwareDoc) {
					const analysisCommands = buildDocAnalysisCommands(
						projectRoot,
						latestHardwareDoc,
					);
			...
			checks.push(
				createCheck(
					...
					localProjectSupport
						...
						: syncedProjectSources.length > 0
							? syncedProjectSources.map(
									(item) =>
										`source=${item.name}, files=${item.targets.project.files_count}`,
								)
							: enabledSources.length > 0
								? enabledSources.map((item) => `source=${item.name}`)
								: [],
			...
			if (
				...
				pushNextCommand(
					...
					hardwareIdentity.model
						? "Install matching chip support into the current project"
						: "Install registered chip support into the current project",
					hardwareIdentity.model
					...
					createCheck(
						...
						matchedProjectSources.length > 0
							...
							: localProjectSupport
								? [
										`chip=${localProjectSupport.name}`,
										`path=${path.relative(projectRoot, localProjectSupport.path).replace(/\\/g, "/")}`,
									]
								: syncedProjectSources.length > 0
									...
									: [
											`model=${hardwareIdentity.model}`,
											`package=${hardwareIdentity.package || "(empty)"}`,
										],
						matchedProjectSources.length > 0 || localProjectSupport
							? ""
							: syncedProjectSources.length > 0
				...
				checks.push(
					createCheck(
						...
						[
							...
							latestHardwareDoc.source
								? `source=${latestHardwareDoc.source}`
								: "",
							latestHardwareDoc.cached_at
								? `cached_at=${latestHardwareDoc.cached_at}`
								: "",
							analysisCommands && analysisCommands.artifact_path
								? `analysis=${analysisCommands.artifact_path}`
								: "",
						],
			...
			if ((normalizedSession.open_questions || []).length > 0) {
				checks.push(
					createCheck(
						...
						(normalizedSession.open_questions || [])
							.slice(0, 4)
		...
		const adapterReusability =
			adapterQualityHelpers.summarizeAdapterReusability(adapterHealth);
		const primaryRecommendation = adapterHealth.primary
			? toolRecommendations.find(
					(item) => item.tool === adapterHealth.primary.tool,
				) || toolRecommendations[0]
			: toolRecommendations[0];
		const primaryToolExecution = primaryRecommendation
			...
			checks.push(
				createCheck(
					...
					adapterHealth.binding_ready_tools > 0 &&
						adapterHealth.draft_binding_tools === 0
						? "pass"
						: "warn",
					adapterHealth.binding_ready_tools > 0
			...
			checks.push(
				createCheck(
					...
					[
						...
						adapterReusability.primary_tool
							? `tool=${adapterReusability.primary_tool}`
							: "",
					].filter(Boolean),
					adapterReusability.status === "reusable"
						? ""
		...
		return {
			...
			bootstrap,
			action_card:
				bootstrap.action_card || buildActionCardFromBootstrap(bootstrap),
	...
	function buildUpdateView(forceCheck) {
		...
		const projectExtDir = getProjectExtDir();
		const statePaths = getProjectStatePaths();
		const projectDataPath = runtime.resolveProjectDataPath(
			projectRoot,
			"project.json",
		);
		const hadProjectLayout =
			fs.existsSync(projectExtDir) || fs.existsSync(projectDataPath);
		const workflowLayout = hadProjectLayout
			? workflowRegistry.syncProjectWorkflowLayout(projectExtDir, {
					write: false,
				})
			: {
		...
		const cachePath = updateCheckHelpers.getUpdateCachePath(
			path,
			RUNTIME_HOST.stateRoot,
		);
		const installed = updateCheckHelpers.readInstalledVersion(
			fs,
			path,
			RUNTIME_HOST.runtimeRoot,
		);
		const hookVersion =
			process.env.EMB_AGENT_FORCE_HOOK_VERSION || readHookVersion();
		const cache = updateCheckHelpers.readUpdateCache(fs, cachePath);
		const staleInstall = updateCheckHelpers.detectStaleInstall(
			installed,
			hookVersion,
		);
		...
		const latestCache =
			updateCheckHelpers.readUpdateCache(fs, cachePath) || cache;

		const recommendations = [];
		if (staleInstall) {
			recommendations.push(
				"Re-run emb-agent install first to align hooks / runtime / agents versions.",
			);
		}
		if (latestCache && latestCache.update_available && latestCache.latest) {
			recommendations.push(
				"A new version was detected. Read the release notes first, then reinstall runtime.",
			);
		}
		if (trigger.triggered) {
			recommendations.push(
				"A background version check has been triggered. Run update again later to see the latest result.",
			);
		}
		if (recommendations.length === 0) {
			recommendations.push(
				"There is no explicit upgrade blocker right now. Run update check if you want to confirm the latest version.",
			);
		}
		if (workflowLayout.migrated.length > 0) {
			recommendations.push(
				"Legacy workflow registry paths were migrated into .emb-agent/registry/workflow.json.",
			);
		}
		if (workflowLayout.created.length > 0) {
			recommendations.push(
				"Project workflow layout was normalized so spec/template/registry paths now share the same root.",
			);
		}
		...
		return {
			...
			check: {
				...
				stale: updateCheckHelpers.isUpdateCacheStale(
					cache,
