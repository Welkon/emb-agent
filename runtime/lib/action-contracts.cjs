...
function createActionContractHelpers(deps) {
	...
	function buildActionFollowup(action, resolved, activeTask) {
		const scanWorkflowStage =
			action === "scan" && typeof buildWorkflowStage === "function"
		...
		if (action === "ai-host explore-prd") {
			return {
				label: "Ask PRD exploration questions",
				cli: "",
				followup:
					"Then: update docs/prd/system.md, .emb-agent/req.yaml, and child execution PRDs before prd confirm --create-tasks",
			};
		}
	...
	function buildActionSummary(action) {
		switch (action) {
			...
			case "ai-host explore-prd":
				return "Action=prd-exploration. Ask detailed requirements questions, sync req.yaml, and create child PRDs before confirmation.";
			default:
				return "";
		}
	}
	...
	function buildActionReason(action, output, resolved) {
		...
		if (action === "ai-host explore-prd") {
			return (
				output.summary ||
				output.problem ||
				"PRD exploration is required before confirmation"
			);
		}
	...
	function buildCodeWritingSpecsInstruction(output) {
		const codeWritingSpecs =
			...
			output.code_writing_specs.status === "required"
				? output.code_writing_specs
				: null;
		if (
			!codeWritingSpecs ||
			!Array.isArray(codeWritingSpecs.items) ||
			codeWritingSpecs.items.length === 0
		) {
			return "";
		}
	...
	function buildActionInstruction(action, output) {
		if (action === "scan") {
			return (
				(output.next_reads && output.next_reads[0]) ||
				(output.open_questions && output.open_questions[0]) ||
				""
			);
		}
		if (action === "plan") {
			return (
				...
				(output.verification && output.verification[0]) ||
				""
			);
		}
		if (action === "do") {
			...
			const workInstruction =
				(output.execution_brief &&
					output.execution_brief.suggested_steps &&
					output.execution_brief.suggested_steps[0]) ||
		...
		if (action === "review") {
			return (
				(output.required_checks && output.required_checks[0]) ||
				(output.findings_template && output.findings_template[0]) ||
				""
			);
		}
		if (action === "verify") {
			return (
				(output.checklist && output.checklist[0]) || output.next_step || ""
			);
		}
		if (action === "forensics") {
			return (
				(output.evidence_sources && output.evidence_sources[0]) ||
				output.next_step ||
				""
			);
		}
	...
	function buildActionNextActions(action, actionCard, output) {
		return runtime.unique([
			actionCard && actionCard.first_instruction
				? `instruction=${actionCard.first_instruction}`
				: "",
			actionCard && actionCard.first_cli
				? `command=${actionCard.first_cli}`
				: "",
			actionCard && actionCard.followup
				? `followup=${actionCard.followup}`
				: "",
			output &&
			output.code_writing_specs &&
			output.code_writing_specs.status === "required"
				? `required_code_writing_specs=${(output.code_writing_specs.items || [])
						.map((item) => item.path)
						.filter(Boolean)
						.join(",")}`
				: "",
	...
	function buildWorkerContract(call, defaults = {}) {
		return {
			goal: String(
				defaults.goal || call.purpose || "Execute the assigned worker task",
			).trim(),
	...
	function buildReviewContract() {
		return {
			required: true,
			policy:
				"If Stage A fails, set redispatch_required=true and tighten the worker contract instead of patching inline in the main thread.",
			stage_a: {
				id: "contract-review",
				owner: "Current main thread",
				objective:
					"Verify architecture worker outputs match the worker contract and stay inside declared read-only boundaries.",
				completion_signal:
					"contract compliance, evidence boundaries, and drive-by changes are explicit",
			...
			stage_b: {
				...
				owner: "Current main thread",
				objective:
					"Review architecture quality, tradeoff coherence, residual risks, and follow-up gaps only after Stage A passes.",
				completion_signal:
					"quality findings and merge/reject decision are explicit",
	...
	function buildInjectedSpecs(
		...
		const snapshot = workflowRegistry.buildInjectedSpecSnapshot(
			...
			},
			{
				...
				auto_required_enforcement_scope:
					settings.auto_required_enforcement_scope,
			},
		);
	...
	function buildCodeWritingSpecContract(injectedSpecs) {
		const requiredSpecs = (
			Array.isArray(injectedSpecs) ? injectedSpecs : []
		).filter(
			(item) =>
				item &&
				item.required === true &&
		...
		return {
			...
			summary:
				"Read and obey these active code-writing specs before editing, generating, or refactoring firmware/source code.",
	...
	function buildActionOutput(action) {
		...
		const codeWritingSpecs =
			action === "do" ? buildCodeWritingSpecContract(injectedSpecs) : null;
		const workflowStage =
			typeof buildWorkflowStage === "function"
				? buildWorkflowStage({ command: action }, resolved)
				: null;
		const capabilityDefinition = capabilityCatalog.getCapabilityDefinition(
			action,
			{
			...
			output = {
				...
				scheduler: {
					...
					agent_execution: {
						...
						reason:
							"Health is a read-only self-check action and should run inline on the current main thread by default.",
						primary_agent: "",
						supporting_agents: [],
		...
		} else if (action === "ai-host explore-prd") {
			const projectRoot =
				resolved && resolved.session ? resolved.session.project_root : "";
			const prdState = projectRoot
				? prdCommand.summarizePrdConfirmationState(projectRoot, {
						fs,
						path,
						runtime,
					})
				: null;
			const exploration =
				prdState && prdState.exploration ? prdState.exploration : {};
			output = {
				...
				summary:
					exploration.summary ||
					"Ask detailed PRD exploration questions before confirming tasks.",
				evidence_sources: runtime.unique([
					exploration.system_prd_path || "docs/prd/system.md",
					exploration.requirement_truth && exploration.requirement_truth.path
						? exploration.requirement_truth.path
						: ".emb-agent/req.yaml",
				]),
				findings_template: Array.isArray(exploration.prompts)
					? exploration.prompts
					: [],
				required_updates: Array.isArray(exploration.required_updates)
					? exploration.required_updates
					: [],
				child_prd_locations: Array.isArray(exploration.child_prd_locations)
					? exploration.child_prd_locations
					: [],
				next_step:
					"Ask the user about behavior, constraints, acceptance, and unclear hardware facts; then update PRD, req.yaml, and child PRDs before confirmation.",
				...
				scheduler: {
					...
					agent_execution: {
						...
						mode: "inline-preferred",
						reason:
							"PRD exploration is a human-alignment gate and should be handled by the host AI in the current conversation.",
						primary_agent: "",
						supporting_agents: [],
						dispatch_contract: null,
					},
		...
		const enriched = enrichWithToolSuggestions(
			{
				...
				agent_execution:
					output.scheduler && output.scheduler.agent_execution
	...
	function buildArchReviewDispatchContext() {
		...
		return {
			...
			agent_execution: {
				...
				reason:
					"An explicit architecture preflight should be led directly by emb-arch-reviewer.",
				...
				dispatch_contract: {
					...
					phases: [
						{
							...
							objective:
								"Gather architecture options, constraints, and pre-mortem evidence",
						...
						},
						{
							...
							owner: "Current main thread",
							objective:
								"Compose a self-contained architecture decision brief before any downstream conclusion",
							completion_signal:
								"the next consumer can decide without seeing prior conversation",
						},
						{
							id: "execution",
							owner: context.suggested_agent,
							objective:
								"Produce the primary architecture review conclusion against the synthesized brief",
							completion_signal:
								"evaluation matrix and pre-mortem are explicit",
						},
						{
							id: "integration",
							owner: "Current main thread",
							objective:
								"Integrate the review conclusion and decide the next project action",
					...
					primary: {
						...
						purpose:
							"Execute system-level architecture preflight, option comparison, and pre-mortem",
						ownership:
							"Own the primary review conclusion and do not replace concrete implementation changes",
					...
					supporting: runtime
						...
						.map((agent) => ({
							...
							purpose:
								"Add structural, hardware, or release-side evidence for the architecture preflight",
							ownership:
								"Add side evidence only and do not override the primary review conclusion",
							...
							expected_output: [
								"Supplemental evidence, constraints, or risks awaiting verification",
							],
							worker_contract: buildWorkerContract(
								{
									purpose:
