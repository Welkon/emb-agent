...
function createSessionFlowHelpers(deps) {
	...
	const { analyzeIntentSelection, normalizeIntentRouterConfig } =
	...
	function getReviewedTranscriptNext(session) {
		...
		const recommended =
			...
			typeof latest.recommended_next === "object"
				...
				: null;
		const route =
			recommended && recommended.route ? String(recommended.route) : "";

		if (
			!latest ||
			!review ||
			review.status !== "accepted" ||
			!lastCommandStartsWith(session, "transcript apply") ||
			!route
		) {
			return null;
		}
	...
	function formatCapabilityFlow(names) {
		return names
			.map(formatPreferredCapabilityCommand)
			.filter(Boolean)
			.join(" -> ");
	}
	...
	function isBlankProjectSelectionMode(resolved) {
		const projectRoot =
			resolved && resolved.session ? resolved.session.project_root : "";
		const hardware = resolved && resolved.hardware ? resolved.hardware : {};
		const identity =
			hardware && hardware.identity ? hardware.identity : hardware;
		const cacheKey = JSON.stringify({
			projectRoot,
			model: identity && identity.model ? identity.model : "",
			package: identity && identity.package ? identity.package : "",
			chipProfile:
				hardware && hardware.chip_profile
					? hardware.chip_profile.name || "present"
					: "",
		});
	...
	function buildPrdConfirmationNext(resolved) {
		...
		if (!prdState || prdState.documents.length === 0) {
			return null;
		}

		const exploration =
			prdState.exploration && typeof prdState.exploration === "object"
				? prdState.exploration
				: null;
		const generatedTasks = Array.isArray(prdState.generated_tasks)
			? prdState.generated_tasks
			: [];
		if (
			exploration &&
			exploration.blocks_confirmation &&
			(!prdState.confirmed || generatedTasks.length === 0)
		) {
			return {
				...
				reason:
					exploration.summary ||
					"Explore and align system requirements before PRD confirmation.",
				prd_confirmation: prdState,
				prd_exploration: exploration,
			};
		}

		if (prdState.confirmed) {
			return null;
		}
	...
	function buildTaskSelection(candidates, followupCommand) {
		...
		return {
			...
			candidates: tasks,
			then_cli: followup
				? buildPreferredCapabilityCli(followup)
				: buildPreferredCapabilityCli("scan"),
	...
	function buildTaskConvergenceForNext(resolved, taskLike) {
		...
		const selectionMode = String(
			hardware && hardware.selection_mode ? hardware.selection_mode : "",
		).trim();
		const openQuestions = Array.isArray(task.open_questions)
			? task.open_questions.filter(Boolean)
			: [];
		const knownRisks = Array.isArray(task.known_risks)
			? task.known_risks.filter(Boolean)
			: [];
		const chipUnknown = !String(
			identity.model || taskIdentity.model || "",
		).trim();
		const scanFirst =
			selectionMode === "blank-project" ||
			chipUnknown ||
		...
		return {
			...
			paths: [
				...
				},
				{
					...
					outcome:
						"Execution starts from a locked task plan instead of chat drift.",
				},
				{
					id: "review-before-do",
					...
					outcome:
						"Structural risks are explicit before implementation moves forward.",
				},
			],
	...
	function getRecentSchematicAnalysis(resolved) {
		...
		const parsedFiles = lastFiles.filter((file) =>
			/(?:^|\/)\.emb-agent\/cache\/schematics\/[^/]+\/parsed\.json$/i.test(
				String(file || ""),
			),
		);
		...
		const agentAnalysis =
			...
			typeof summary.agent_analysis === "object"
				? summary.agent_analysis
				: null;
		if (!agentAnalysis) {
			return null;
	...
	function getRecentHardwareDocAnalysis(resolved) {
		...
		const parseFiles = lastFiles.filter((file) =>
			/(?:^|\/)\.emb-agent\/cache\/docs\/[^/]+\/parse\.md$/i.test(
				String(file || ""),
			),
		);
		...
		const recommendedFlow =
			...
			typeof summary.recommended_flow === "object"
				? summary.recommended_flow
				: null;
		const handoffProtocol =
			summary &&
			summary.handoff_protocol &&
			typeof summary.handoff_protocol === "object"
				? summary.handoff_protocol
				: null;
		if (!agentAnalysis) {
			return null;
	...
	function shouldGateNextWithHealth(
		resolved,
		handoff,
		nextCommand,
		healthReport,
		...
		const pendingSourceIntake =
			...
			typeof initGuidance.pending_source_intake === "object"
				? initGuidance.pending_source_intake
				: null;
		...
		if (
			initGuidance &&
			(initGuidance.project_definition_required ||
				initGuidance.hardware_confirmation_required)
		) {
			return false;
		}
		...
		const hasBlockingCheck = Array.isArray(healthReport.checks)
			? healthReport.checks.some(
					(item) =>
						blockingChecks.has(item.key) &&
		...
		const actionableHealthCommands = Array.isArray(healthReport.next_commands)
			? healthReport.next_commands.some((item) =>
					["init", "doc-apply"].includes(item.key),
				)
			: false;
	...
	function getPreferences(session) {
		return runtime.normalizePreferences(
	...
	function collectRoutingTexts(session) {
		const latestForensics =
			session && session.diagnostics && session.diagnostics.latest_forensics
				? session.diagnostics.latest_forensics
				: null;
		const latestExecutor =
			session && session.diagnostics && session.diagnostics.latest_executor
		...
		return runtime
			.unique([
				...
				session && session.active_task && session.active_task.title
					? session.active_task.title
					: "",
				latestForensics && latestForensics.problem
					? latestForensics.problem
					: "",
				latestExecutor && latestExecutor.status
					? `executor ${latestExecutor.name || ""} ${latestExecutor.status}`
					: "",
				latestExecutor && latestExecutor.stderr_preview
					? latestExecutor.stderr_preview
					: "",
				latestExecutor && latestExecutor.stdout_preview
	...
	function hasPattern(texts, patterns) {
		return texts.some((text) =>
			patterns.some((pattern) =>
				String(text).toLowerCase().includes(String(pattern).toLowerCase()),
			),
		);
	}
	...
	function shouldSuggestReview(resolved) {
		...
		const isComplexRuntime =
			resolved.profile.runtime_model !== "main_loop_plus_isr";
		...
		return (
			(isComplexRuntime && hasWideReviewSurface) ||
			(hasReviewSignal && (isComplexRuntime || hasWideReviewSurface))
		);
	}
	...
	function lastCommandIs(session, name) {
		return (
			normalizeLastWorkflowCommand(session && session.last_command) ===
			String(name || "")
				.trim()
				.toLowerCase()
		);
	...
	function lastCommandStartsWith(session, name) {
		const raw = String((session && session.last_command) || "")
			.trim()
			.toLowerCase();
	...
	function buildQualityGateInstallHints(qualityGates) {
		const gates =
			qualityGates && typeof qualityGates === "object" ? qualityGates : {};
		const installed = new Set(listInstalledSkillNames());
		const pendingSkills = Array.isArray(gates.pending_skills)
		...
		return pendingSkills
			...
			.map(
				(name) =>
					`quality_gate_install=${runtimeHostHelpers.buildCliCommand(
						RUNTIME_HOST,
						["skills", "install", "--scope", "project", "--skill", name],
					)}`,
			);
	}
	...
	function shouldSuggestScanTool(resolved) {
		...
		const suggestedTools =
			(resolved.effective && resolved.effective.suggested_tools) || [];

		return (
			hasPattern(texts, HARDWARE_TOOL_PATTERNS) || suggestedTools.length > 0
		);
	}
	...
	function shouldSuggestArchReview(resolved) {
		...
		const texts = runtime
			...
			.filter(Boolean);
		const patterns = runtime
			.unique(resolved.effective.arch_review_triggers || [])
			.filter(Boolean);

		return texts.some((text) =>
			patterns.some((pattern) =>
				text.toLowerCase().includes(String(pattern).toLowerCase()),
			),
		);
	}
	...
	function buildArchReviewContext() {
		...
		const capabilityRoute = capabilityRouter.buildCapabilityRoute(
			"arch-review",
			{
				command: "arch-review",
		...
		return {
			...
			warning:
				"This is an explicit heavyweight review entry point. Use it only for selection reviews, solution preflight, PoC-to-production, or pre-mortem scenarios.",
		};
	}
	...
	function buildNextCommand(resolved, handoff) {
		...
		if (pendingTranscriptReview) {
			return {
				...
				reason:
					`Imported transcript ${pendingTranscriptReview.source_id || pendingTranscriptReview.provider || ""} requires AI semantic review before its signals can steer the workflow.`.trim(),
		...
		if (useBlankSelectionFlow) {
			if (lastCommandStartsWith(session, "do")) {
				return {
					command: "verify",
					reason:
						"A concept-stage do step just finished; verify that the recorded shortlist, constraints, and evidence are explicit before moving on",
				};
			}
			...
			if (preferences.review_mode === "always") {
				return {
					command: "review",
					reason:
						"Review mode is forced. Run review before closing the concept-stage selection pass.",
				};
			}
			...
			if (shouldSuggestArchReview(resolved)) {
				return {
					command: "arch-review",
					reason:
						"Concept-stage preflight signals are active. Run arch-review before narrowing the selection path.",
				};
			}
			...
			if (shouldSuggestReview(resolved)) {
				return {
					command: "review",
					reason:
						"A review signal is active; run a structural review before locking the concept-stage selection path",
				};
			}
			...
			if (
				shouldSuggestPlan(resolved) ||
				lastCommandStartsWith(session, "scan")
			) {
				return {
					command: "plan",
		...
		if (lastCommandIs(session, "do") && hasActiveContext) {
			return {
				command: "verify",
				reason:
					"A do step just finished. Run verify next to close the iteration with a result record.",
			};
		}
		...
		if (shouldSuggestArchReview(resolved)) {
			return {
				command: "arch-review",
				reason:
					"Selection or solution preflight signals are active. Run arch-review before execution.",
			};
		}
		...
		if (
			taskConvergence &&
			taskConvergence.recommended_path === "plan-first" &&
			!peripheralWalkthrough
		) {
			const prdPath =
				taskConvergence.prd_path || buildTaskPrdPath(activeTask.name);
			const taskLabel =
				activeTask && activeTask.title ? activeTask.title : activeTask.name;
			const firstTool = primaryToolRecommendation || suggestedTools[0] || null;
			const toolName =
				firstTool && (firstTool.tool || firstTool.name)
					? firstTool.tool || firstTool.name
					: "";
			return {
				command: "plan",
				reason: toolName
		...
		if (taskConvergence && !peripheralWalkthrough) {
			const prdPath =
				taskConvergence.prd_path || buildTaskPrdPath(activeTask.name);
			const taskLabel =
				activeTask && activeTask.title ? activeTask.title : activeTask.name;
			return {
				command:
					taskConvergence.recommended_path === "scan-first" ? "scan" : "plan",
				reason:
					taskConvergence.recommended_path === "scan-first"
		...
		if (pendingSourceIntake && pendingSourceIntake.command) {
			...
			return {
				command: "scan",
				reason:
					`Detected ${sourceLabel} ${pendingSourceIntake.file || ""}. Close source intake first so scan works from normalized project truth instead of raw files.`.trim(),
			};
		}
		...
		if (
			...
			const identity =
				...
				typeof initGuidance.selected_identity === "object"
					? initGuidance.selected_identity
					: {};
			...
			return {
				...
				task_prompt:
					"Give the task you want done in one sentence. I will create and activate the task before continuing.",
				task_intake: {
					status: "ready",
		...
		if (!hasActiveContext) {
			return {
				command: "scan",
				reason:
					"No effective working context exists yet. Run a minimal scan first.",
			};
		}
		...
		if (lastFiles.length === 0) {
			return {
				command: "scan",
				reason:
					"No recent-file record exists yet. Run scan first to lock onto the real change point.",
			};
		}
		...
		return {
			command: "do",
			reason:
				"Context is already sufficient. Proceed with the minimal execution directly.",
		};
	}
	...
	function buildContextHygiene(resolved, handoff, currentCommand) {
		...
		if (lastFiles.length >= 16) {
			score += 2;
			reasons.push(
				`Recent files have reached ${lastFiles.length}, which means the context span is starting to widen`,
			);
		} else if (lastFiles.length >= 10) {
			score += 1;
			reasons.push(
				`There are already ${lastFiles.length} recent files; close down the scope before digging deeper`,
			);
		}
		...
		if (focus.trim() !== "" && heavyCommands.includes(command)) {
			score += 1;
			reasons.push(
				`The latest command was ${command}, and the session is still digging around the focus`,
			);
		}
		...
		if (handoff) {
			score += 2;
			reasons.push(
				"A pause handoff already exists; you can clear context and resume directly",
			);
		}
		...
		let recommendation =
			"Context load is light. Keep working in the current session.";
		...
		return {
			...
			compress_cli: runtimeHostHelpers.buildCliCommand(RUNTIME_HOST, [
				"context",
				"compress",
			]),
	...
	function getToolRecommendationScore(item) {
		const status = chipSupportStatusHelpers.normalizeChipSupportStatus(
	...
	function collectHardwareIntentTexts(resolved) {
		...
		const identity =
			hardware && hardware.identity ? hardware.identity : hardware;
		const signals = Array.isArray(identity && identity.signals)
			? identity.signals
			: [];
		const peripherals = Array.isArray(identity && identity.peripherals)
			? identity.peripherals
			: [];
		const session = resolved && resolved.session ? resolved.session : {};
		const activeTask = getActiveTask ? getActiveTask() : null;
	...
	function shouldSuggestPeripheralWalkthrough(resolved, toolRecommendations) {
		...
		return /exercise all supported|all supported .*peripheral|all peripheral|all supported tools/.test(
			joined,
		);
	}
	...
	function buildPeripheralWalkthroughActions(resolved, toolRecommendations) {
		...
		const readyItems = (
			Array.isArray(toolRecommendations) ? toolRecommendations : []
	...
	function buildWalkthroughRecommendation(resolved, toolRecommendations) {
		...
		return {
			...
			summary:
				"Walk every ready tool once and capture one concrete output per peripheral.",
	...
	function buildWalkthroughRuntimeActions(session, recommendation) {
		...
		const orderedTools = Array.isArray(recommendation.ordered_tools)
			? recommendation.ordered_tools
			: [];
		const runtimeTools = Array.isArray(runtimeState.ordered_tools)
			? runtimeState.ordered_tools
			: [];
		const sameWalkthrough =
			runtimeState.kind === recommendation.kind &&
		...
		const totalSteps = Number.isInteger(runtimeState.total_steps)
			? runtimeState.total_steps
			: orderedTools.length;
		...
		if (runtimeState.status === "completed") {
			return [
				...
				runtimeState.last_summary
					? `walkthrough_last=${runtimeState.last_summary}`
					: "",
			].filter(Boolean);
		}
		...
		return [
			...
			currentStep &&
			currentStep.status === "needs-input" &&
			Array.isArray(currentStep.missing_inputs) &&
			currentStep.missing_inputs.length > 0
	...
	function buildAdapterHealthHints(healthReport, primaryToolRecommendation) {
		const adapterHealth =
			healthReport && healthReport.chip_support_health
				...
				: null;
		const primary =
			adapterHealth && adapterHealth.primary ? adapterHealth.primary : null;
		const preferredTrustTool =
			...
			primaryToolRecommendation.trust.executable
				? {
						...
						recommended_action:
							primaryToolRecommendation.trust.recommended_action || "",
					}
				: primary;
		const reusability =
			adapterHealth &&
			adapterHealth.reusability &&
			typeof adapterHealth.reusability === "object"
				? adapterHealth.reusability
				: null;
	...
	function selectPrimaryToolRecommendation(toolRecommendations, resolved) {
		const items = Array.isArray(toolRecommendations)
			? toolRecommendations.slice()
			: [];
		if (items.length === 0) {
			return null;
		}
		const rankedIntent = analyzeIntentSelection({
			projectConfig:
				resolved && resolved.project_config ? resolved.project_config : {},
		...
		return items
			...
			.sort((left, right) => {
				const leftIntent = rankedIntent.ranked.find(
					(entry) => entry.item === left.item,
				);
				const rightIntent = rankedIntent.ranked.find(
					(entry) => entry.item === right.item,
				);
				return (
					getToolRecommendationScore(left.item) -
						getToolRecommendationScore(right.item) ||
					((rightIntent && rightIntent.intent_score) || 0) -
						((leftIntent && leftIntent.intent_score) || 0) ||
					Number(
						Boolean(
							right.item && right.item.trust && right.item.trust.executable,
						),
					) -
						Number(
							Boolean(
								left.item && left.item.trust && left.item.trust.executable,
							),
						) ||
					getToolRecommendationTrustScore(right.item) -
						getToolRecommendationTrustScore(left.item) ||
	...
	function buildMemorySummaryArtifact(resolved, handoff, source) {
		...
		return {
			...
			stale_note:
				"This compact snapshot is static and will not auto-update; rerun a recovery pointer to refresh live state.",
			...
			diagnostics: {
				...
				latest_skill: {
					...
					exit_code:
						latestSkill.exit_code === undefined ? null : latestSkill.exit_code,
				...
				latest_executor: {
					...
					exit_code:
						latestExecutor.exit_code === undefined
							? null
							: latestExecutor.exit_code,
	...
	function buildMemorySummaryView(memorySummary) {
		...
		return {
			...
			active_task: memorySummary.active_task || {
				name: "",
				title: "",
				status: "",
				package: "",
			...
			diagnostics: memorySummary.diagnostics || {
				latest_forensics: {},
				latest_skill: {},
				latest_executor: {},
			},
	...
	function buildGuidance(resolved, handoff, memorySummary) {
		...
		const recommendedSources =
			(resolved.effective && resolved.effective.recommended_sources) || [];
		const suggestedTools =
			(resolved.effective && resolved.effective.suggested_tools) || [];
		const toolRecommendations =
			(resolved.effective && resolved.effective.tool_recommendations) || [];
		...
		const peripheralWalkthroughActions = buildPeripheralWalkthroughActions(
			resolved,
			toolRecommendations,
		);
		const walkthroughRecommendation = buildWalkthroughRecommendation(
			resolved,
			toolRecommendations,
		);
		const walkthroughRuntimeActions = buildWalkthroughRuntimeActions(
			session,
			walkthroughRecommendation,
		);
		const primaryRegisterSource =
			recommendedSources.find(
				(item) => item.priority_group === "register-summary",
			) || null;
		const primarySource =
			primaryRegisterSource || recommendedSources[0] || null;
		const latestForensics =
			session.diagnostics && session.diagnostics.latest_forensics
				? session.diagnostics.latest_forensics
				: null;
		...
		const summaryLastFiles =
			memorySummary &&
			Array.isArray(memorySummary.last_files) &&
			memorySummary.last_files.length > 0
				? memorySummary.last_files
				: lastFiles;
		const summaryOpenQuestions =
			memorySummary &&
			Array.isArray(memorySummary.open_questions) &&
			memorySummary.open_questions.length > 0
				? memorySummary.open_questions
				: openQuestions;
		const summaryKnownRisks =
			memorySummary &&
			Array.isArray(memorySummary.known_risks) &&
			memorySummary.known_risks.length > 0
		...
		return {
			...
			next_actions: runtime.unique([
				memorySummary && memorySummary.generated_at
					...
					: "",
				!memorySummary &&
				["consider-clearing", "suggest-clearing"].includes(contextHygiene.level)
				...
				handoff && handoff.next_action
					? `handoff_resume=${handoff.next_action}`
					: "",
				...(handoff
					? handoff.human_actions_pending.map(
							(action) => `manual_action=${action}`,
						)
					: []),
				`system_prd=${systemPrd.getSystemPrdRelativePath(runtime)}`,
				summaryTask
					? `task_resume=${summaryTask.name}; title=${summaryTask.title}`
					: "",
				taskConvergence && taskConvergence.prd_path
					? `task_prd=${taskConvergence.prd_path}`
					: "",
				taskConvergence && taskConvergence.summary
					? `task_convergence=${taskConvergence.summary}`
					: "",
				taskConvergence && taskConvergence.recommended_path
					...
					: "",
				taskConvergence && taskConvergence.next_cli
					? `task_next=${taskConvergence.next_cli}`
					: "",
				taskConvergence && taskConvergence.then_cli
					? `task_then=${taskConvergence.then_cli}`
					: "",
				taskConvergence && taskConvergence.review_hint
					? `task_review=${taskConvergence.review_hint}`
					: "",
				next && next.prd_confirmation && next.prd_confirmation.status
					...
					: "",
				next && next.prd_exploration && next.prd_exploration.status
					? `prd_exploration=${next.prd_exploration.status}; updates=${(next.prd_exploration.required_updates || []).join(" | ")}`
					: "",
				...(next &&
				next.prd_exploration &&
				Array.isArray(next.prd_exploration.prompts)
					? next.prd_exploration.prompts
							.slice(0, 3)
							.map((item) => `prd_question=${item}`)
					: []),
				next && next.task_selection && next.task_selection.summary
					? `task_selection=${next.task_selection.summary}`
					: "",
				...(next &&
				next.task_selection &&
				Array.isArray(next.task_selection.candidates)
					? next.task_selection.candidates
							.slice(0, 5)
							.map(
								(task) =>
				...
				summaryLatestExecutor && summaryLatestExecutor.name
					? `executor_status=${summaryLatestExecutor.name}; status=${summaryLatestExecutor.status || "unknown"}${
							summaryLatestExecutor.exit_code === null
								? ""
								: `, exit=${summaryLatestExecutor.exit_code}`
						}`
					: "",
				summaryLatestSkill && summaryLatestSkill.name
					? `skill_status=${summaryLatestSkill.name}; status=${summaryLatestSkill.status || "unknown"}${
							summaryLatestSkill.exit_code === null
								? ""
								: `, exit=${summaryLatestSkill.exit_code}`
						}`
					: "",
				summaryLatestExecutor &&
				["failed", "error"].includes(summaryLatestExecutor.status)
					? `executor_review=${summaryLatestExecutor.name}${summaryLatestExecutor.stderr_preview ? ` | ${summaryLatestExecutor.stderr_preview}` : ""}`
					: "",
				summaryLatestSkill &&
				["failed", "error", "blocked"].includes(summaryLatestSkill.status)
				...
				qualityGates.enabled
					...
					: "",
				qualityGates.blocking_summary &&
				qualityGates.blocking_summary !== qualityGates.status_summary
					? `quality_gate_blocking=${qualityGates.blocking_summary}`
					: "",
				...qualityGateInstallHints,
				...qualityGates.recommended_runs.map(
					(item) => `quality_gate_run=${item}`,
				),
				...qualityGates.recommended_signoffs.map(
					(item) => `quality_gate_signoff=${item}`,
				),
				...qualityGates.rejected_signoffs.map(
					(item) => `quality_gate_rejected=${item}`,
				),
				...
				hardwareDocAnalysis && hardwareDocAnalysis.recommended_agent
					...
					: "",
				hardwareDocAnalysis &&
				hardwareDocAnalysis.confirmation_targets.length > 0
				...
				hardwareDocAnalysis && hardwareDocAnalysis.cli_hint
					...
					: "",
				primaryRegisterSource
					? `reread_register_summary=${primaryRegisterSource.path}`
					: "",
				!primaryRegisterSource && primarySource
					? `reread_source_summary=${primarySource.path}`
					: "",
				...suggestedTools
					.slice(0, 2)
					.map((tool) => `tool_candidate=${tool.name}; status=${tool.status}`),
				primaryToolRecommendation
					? `tool_cli=${primaryToolRecommendation.cli_draft}`
					: "",
				primaryToolRecommendation &&
				(primaryToolRecommendation.missing_inputs || []).length > 0
					? `tool_missing_inputs=${primaryToolRecommendation.missing_inputs.join(", ")}`
					: "",
				focus ? `focus=${focus}` : "",
				summaryLastFiles[0] ? `reread_file=${summaryLastFiles[0]}` : "",
				summaryOpenQuestions[0]
					? `open_question=${summaryOpenQuestions[0]}`
					: "",
	...
	function buildWorkflowStage(nextCommand, resolved) {
		const command = String(
			nextCommand && nextCommand.command ? nextCommand.command : "",
		).trim();
		const blankSelectionMode = isBlankProjectSelectionMode(resolved);
		...
		if (command === "health") {
			return {
				...
				exit_criteria:
					"Health next commands are closed and the next command is no longer health",
		...
		if (command === "transcript-review") {
			return {
				...
				exit_criteria:
					"Transcript analysis is marked ai-reviewed or intentionally bypassed with an explicit unreviewed apply",
		...
		if (command.startsWith("task activate ")) {
			return {
				...
				exit_criteria:
					"One existing open task is activated before scan, plan, or mutation",
				primary_command: "task activate",
			};
		}

		if (command === "ai-host explore-prd") {
			return {
				name: "prd-exploration",
				why: "System PRD needs requirement exploration, structured req.yaml sync, and child execution PRDs before confirmation",
				exit_criteria:
					"User-visible behavior and constraints are clarified, req.yaml is non-template, and child PRDs exist",
				primary_command: "ai-host explore-prd",
			};
		}
		...
		if (command.startsWith("prd confirm")) {
			return {
				...
				exit_criteria:
					"PRD manifest is confirmed and concrete execution tasks exist",
		...
		if (command === "task add <summary>" || command === "task add") {
			return {
				...
				exit_criteria:
					"A concrete task is created and activated before scan, plan, or mutation",
		...
		if (["review", "verify"].includes(command)) {
			return {
				...
				exit_criteria:
					"Required checks are captured and no blocking risk remains",
				primary_command: command,
			};
		}
		...
		return {
			...
			exit_criteria:
				"Implementation change or debug result is produced with evidence",
		...
		return enrichWithToolSuggestions(
			{
				...
				task: activeTask
					? {
							...
							injected_specs:
								Array.isArray(activeTask.injected_specs) &&
								activeTask.injected_specs.length > 0
	...
	function buildNextActionCard(
		...
		function isHousekeepingHint(text) {
			return /^(snapshot_command=|task_resume=|summary_resume=|handoff_resume=|context=|chip_support_status=|chip_support_trust=|reread_)/i.test(
				String(text || "").trim(),
			);
		}
		...
		function scoreActionHint(text, command) {
			...
			const commandName = String(
				...
				.toLowerCase();
			const reason = String(
				command && command.reason ? command.reason : "",
			).toLowerCase();
			const walkthroughMode =
				commandName === "scan" &&
				(reason.includes("broad peripheral exercise") ||
					reason.includes("broad peripheral walkthrough"));
			...
			if (normalized.startsWith("ready_tool_checklist="))
				return walkthroughMode ? 1 : 4;
			if (normalized.startsWith("walkthrough_plan="))
				return walkthroughMode ? 3 : 5;
			if (normalized.includes("do not stop at the first matching tool"))
				return 0;
		...
		const command =
			nextCommand && typeof nextCommand === "object" ? nextCommand : {};
		const stage =
			workflowStage && typeof workflowStage === "object" ? workflowStage : {};
		const quickstart =
			command.health_quickstart && typeof command.health_quickstart === "object"
				? command.health_quickstart
				: {};
		const firstQuickstartStep =
			Array.isArray(quickstart.steps) && quickstart.steps.length > 0
				? quickstart.steps[0]
				: null;
		const followupCli = String(quickstart.followup || "")
			.replace(/^Then:\s*/i, "")
			.trim();
		const taskIntakeCommand =
			command.command === "task add <summary>" ||
			command.command === "task add";
		const prdExplorationCommand = command.command === "ai-host explore-prd";
		const prdConfirmationCommand = String(command.command || "").startsWith(
			"prd confirm",
		);
		const taskActivateCli =
			command.task_intake && command.task_intake.then_cli
		...
		const actionName = command.gated_by_health
			...
			: taskIntakeCommand
				? "Create a task"
				: prdExplorationCommand
					? "Explore PRD requirements"
		...
		const firstLabel = command.gated_by_health
			? firstQuickstartStep && firstQuickstartStep.label
				? firstQuickstartStep.label
				: "Run health closure first"
			: taskIntakeCommand
				? "Give the task"
				: prdExplorationCommand
					? "Ask PRD questions"
		...
		const firstInstruction = command.gated_by_health
			...
			: taskIntakeCommand
				? command.task_prompt ||
					"Give the task you want done in one sentence, then activate it before continuing."
				: prdExplorationCommand
					? "Before confirming PRD, ask the user about product behavior, boundaries, constraints, acceptance, and unclear hardware/pin facts; then update docs/prd/system.md, .emb-agent/req.yaml, and child execution PRDs."
					: prdConfirmationCommand
		...
		const firstCli =
			firstQuickstartStep && firstQuickstartStep.cli
	...
	function buildNextContext() {
		...
		const gatedByHealth =
			guidance.next.command === "transcript-review"
				? false
				: shouldGateNextWithHealth(
						resolved,
						handoff,
						guidance.next.command,
						health,
					);
		const healthQuickstart =
			health && health.quickstart ? health.quickstart : null;
		...
		const adapterHealthHints = buildAdapterHealthHints(
			health,
			guidance.primary_tool_recommendation,
		);
		...
		const qualityGates = getQualityGateSummary(resolved);
		const boardEvidenceSummary = boardEvidence.summarizeBoardEvidence(
			resolved.session.project_root,
			{
				limit: 8,
			},
		);
		const knowledgeGraph = knowledgeGraphState.summarizeKnowledgeGraph(
			resolved.session.project_root,
			{
				fs,
				path,
		...
		const permissionGateSummary =
			permissionGateHelpers.summarizePermissionGates(permissionGates);
		const injectedSpecs = buildInjectedSpecs(resolved, activeTask, handoff);
		const nextCli =
			nextCommand.command === "ai-host explore-prd"
				? ""
				: nextCommand.cli || buildPreferredCapabilityCli(nextCommand.command);
		const capabilityRoute = capabilityRouter.buildCapabilityRoute(
			nextCommand.command,
			{
		...
		const productLayer = productLayers.summarizeProductLayer(
			nextCommand.command,
		);
		const result = enrichWithToolSuggestions(
			{
				...
				next: {
					...
					recommended_flow:
						guidance.hardware_doc_analysis &&
				...
				optional_evidence_actions: runtime.unique([
					boardEvidenceSummary.command
						? `board_evidence=${boardEvidenceSummary.command}`
						: "",
					boardEvidenceSummary.optional_next_step
						? `board_evidence_note=${boardEvidenceSummary.optional_next_step}`
						: "",
				]),
				hardware_doc_analysis: guidance.hardware_doc_analysis,
				recommended_flow:
	...
	function buildStatus() {
		const resolved = resolveSession();
		const runtimeHost =
			typeof getRuntimeHost === "function"
				? getRuntimeHost()
				: { name: "", subagentBridge: {} };
		...
		const statusCli = runtimeHostHelpers.buildCliCommand(RUNTIME_HOST, [
			"status",
		]);
		const nextActionCli =
			nextAction && nextAction.command
				...
				: "";
		const statusCapabilityRoute = capabilityRouter.buildCapabilityRoute(
			"status",
			{
				command: "status",
				cli: statusCli,
		...
		const nextCapabilityRoute =
			nextAction && nextAction.command
				? capabilityRouter.buildCapabilityRoute(nextAction.command, {
						...
						primary_agent:
							nextAction.command === "arch-review" ? "emb-arch-reviewer" : "",
					})
				: null;
		const statePaths =
			typeof getProjectStatePaths === "function"
				? getProjectStatePaths()
				: null;
