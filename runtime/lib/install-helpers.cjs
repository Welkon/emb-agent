...
function createInstallHelpers(deps) {
	...
	const DEFAULT_SUBAGENT_BRIDGE_TIMEOUT_MS =
		Number(runtimeHost && runtimeHost.DEFAULT_SUBAGENT_BRIDGE_TIMEOUT_MS) ||
		15000;
	...
	const INTERACTIVE_SELECTION_CHANGE_SOURCE = Symbol(
		"interactive-selection-change-source",
	);
	...
	function parseArgs(argv) {
		...
		function setRuntime(name) {
			if (result.runtime && result.runtime !== name) {
				throw new Error(
					`Use one runtime target, received both ${result.runtime} and ${name}`,
				);
			}
			result.runtime = name;
		}
		...
		for (let index = 0; index < argv.length; index += 1) {
			...
			if (token.startsWith("--color=")) {
				result.color = normalizeColorMode(
			...
			if (token === "--subagent-bridge-timeout-ms") {
				result.subagentBridgeTimeoutMs = parsePositiveInteger(
					argv[index + 1],
			...
			if (token === "--default-chip-support-source-location") {
				result.defaultAdapterSourceLocation = String(
					argv[index + 1] || "",
				).trim();
				index += 1;
				continue;
			...
			if (token === "--default-chip-support-source-branch") {
				result.defaultAdapterSourceBranch = String(
					argv[index + 1] || "",
				).trim();
				index += 1;
				continue;
			...
			if (token === "--default-chip-support-source-subdir") {
				result.defaultAdapterSourceSubdir = String(
					argv[index + 1] || "",
				).trim();
				index += 1;
				continue;
		...
		if (
			argv.includes("--subagent-bridge-timeout-ms") &&
			!result.subagentBridgeCmd
		) {
			throw new Error(
		...
		if (
			argv.includes("--default-chip-support-source-location") &&
			!result.defaultAdapterSourceLocation
		) {
			throw new Error(
		...
		if (
			argv.includes("--default-chip-support-source-branch") &&
			!result.defaultAdapterSourceBranch
		) {
			throw new Error(
				"Missing value after --default-chip-support-source-branch",
			);
		}
		if (
			argv.includes("--default-chip-support-source-subdir") &&
			!result.defaultAdapterSourceSubdir
		) {
			throw new Error(
				"Missing value after --default-chip-support-source-subdir",
			);
		}
		if (
			argv.includes("--skill-source") &&
			result.skillSources.some((item) => !item)
		) {
		...
		if (
			(argv.includes("--registry") || argv.includes("-r")) &&
			!result.registry
		) {
		...
		if (!result.uninstall && !result.help && !result.developer) {
			throw new Error(
				"Developer name is required during install. Pass --developer <name>.",
			);
		}
	...
	function buildPromptHeader(chalk) {
		return [
			...
			chalk.gray(
				"  Embedded workflow bootstrap for Codex, Claude Code, Cursor, and Pi",
			),
			"",
		];
	}
	...
	function renderInteractiveSection(
		...
		const lines = [chalk.blue(`${icon} ${title}`)];
	...
	function buildInteractiveRuntimePrompt(targets) {
		...
		for (let index = 0; index < targets.length; index += 1) {
			...
			const globalDir = path.join(
				os.homedir(),
				...(target.defaultGlobalDirParts || [target.localDirName]),
			);
			choiceLines.push(
	...
	function buildInteractiveLocationPrompt(target) {
		...
		const recommended =
			defaultLocation === "local" ? "  Recommended for this runtime" : "";
	...
	function buildInteractiveWorkflowSourcePrompt(
		defaultSource = resolveInstallerDefaultWorkflowSource(),
	) {
		const { chalk } = createPromptStyler();
		const defaultTarget = [
	...
	function buildInteractiveWorkflowSelectionPrompt(preview, options = {}) {
		...
		return buildInteractiveNamedSelectionPrompt({
			...
			context_value:
				preview && preview.source && preview.source.location
			...
			source_change_instruction:
				"Type `source` to use a different spec source.",
			allow_source_change: options.allow_source_change,
		});
	}

	function buildInteractiveSkillSelectionPrompt(preview, options = {}) {
		const skills = Array.isArray(preview && preview.skills)
			? preview.skills
			: [];
		return buildInteractiveNamedSelectionPrompt({
			...
			context_value:
				preview && preview.plugin && preview.plugin.name
					? preview.plugin.name
					: "skill bundle",
			entries: skills,
			...
			source_change_instruction:
				"Type `source` to use a different skill bundle source.",
			allow_source_change: options.allow_source_change,
		});
	}
	...
	function buildInteractiveNamedSelectionPrompt(options = {}) {
		...
		const detailKey =
			String(options.detail_key || "description").trim() || "description";
		...
		const choiceLines = [
			...
			...entries.map((entry, index) => {
				const detail = summarizeInteractiveSkillSelectionDescription(
					entry && entry[detailKey],
				);
		...
		const descriptionLines = [
			...
			String(
				options.empty_instruction || "Press enter or type `skip` to skip.",
			),
		...
		if (allowSourceChange) {
			descriptionLines.push(
				String(
					options.source_change_instruction ||
						"Type `source` to use a different source.",
				),
			);
		}
		...
		return renderInteractiveSection(
			...
			choiceLines,
			allowSourceChange
				? "Choice [skip/all/source] > "
				: "Choice [skip/all] > ",
		);
	}

	function summarizeInteractiveSkillSelectionDescription(
		value,
		maxLength = 140,
	) {
		const normalized = String(value || "")
		...
		const preferred =
			sentenceMatch && sentenceMatch[1] && sentenceMatch[1].length <= maxLength
				? sentenceMatch[1]
				: normalized;
	...
	function resolveInstallerDefaultWorkflowSource() {
		...
		return defaultWorkflowSourceHelpers.resolveDefaultWorkflowSource(
			runtimeConfig,
			process.env,
		);
	}
	...
	function resolveInteractiveWorkflowArgs(prompted) {
		const installSpecsProvided =
			prompted &&
			(Object.hasOwn(prompted, "installSpecs") ||
				Object.hasOwn(prompted, "installWorkflows"));
		const explicitRegistry = String(
			prompted && prompted.registry ? prompted.registry : "",
		).trim();
		...
		const requestedSpecs = normalizeInteractiveWorkflowSpecs(
			prompted && prompted.specs,
		);
		const installSpecs = installSpecsProvided
			...
			: Boolean(
					explicitRegistry ||
						explicitRegistryBranch ||
						explicitRegistrySubdir ||
		...
		const resolvedSource = buildInstallerWorkflowSourceConfig(
			explicitRegistry,
			{
	...
	function resolveInteractiveSkillArgs(prompted) {
		...
		const requestedSkillSources = normalizeInteractiveSkillSources(
			prompted && prompted.skillSources,
		);
		const requestedSkillNames = normalizeInteractiveSkillNames(
			prompted && prompted.skillNames,
		);
		...
		return {
			skillSources:
				requestedSkillSources.length > 0
					? requestedSkillSources
	...
	function parseInteractiveNamedSelection(value, entries, options = {}) {
		...
		const itemLabel =
			String(options.item_label || "selection").trim() || "selection";
	...
	function promptInteractiveSelectionWithKeys(entries, renderUi, options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			function handleKeypress(chunk) {
				...
				if (input === "\u001b[A") {
					state.cursorIndex =
						(state.cursorIndex - 1 + totalChoices) % totalChoices;
					render();
					return;
				}
				if (input === "\u001b[B") {
	...
	function renderInteractiveNamedSelectionKeyboardUi(options = {}, state) {
		...
		const allowSourceChange = Boolean(options.allow_source_change);
		const selected =
			state && state.selected instanceof Set ? state.selected : new Set();
		const cursorIndex = Number.isInteger(state && state.cursorIndex)
			? state.cursorIndex
			: 0;
		...
		const choiceLines = [
			(() => {
				...
				const renderedLabel = isActive
					? chalk.bold(
							chalk.white(
								String(options.skip_label || "Skip").trim() || "Skip",
							),
						)
			...
			...entries.map((entry, index) => {
				...
				const renderedName = isActive
					? chalk.bold(chalk.white(entry.name))
		...
		const descriptionLines = [
			...
			String(
				options.toggle_all_instruction || "Press `a` to toggle every entry.",
			),
		];
		if (allowSourceChange) {
			descriptionLines.push(
				String(
					options.source_change_instruction ||
						"Press `s` to use a different source.",
				),
			);
		}
		descriptionLines.push(
			"Press `Esc`, `Ctrl+C`, or `Ctrl+D` to cancel installation.",
		);
	...
	function renderInteractiveWorkflowSelectionKeyboardUi(
		preview,
		state,
		options = {},
	) {
		...
		return renderInteractiveNamedSelectionKeyboardUi(
			{
				...
				toggle_all_instruction:
					"Press `a` to toggle every selectable spec from that source.",
	...
	function renderInteractiveSkillSelectionKeyboardUi(
		preview,
		state,
		options = {},
	) {
		...
		return renderInteractiveNamedSelectionKeyboardUi(
			{
				...
				source_change_instruction:
					"Press `s` to use a different skill bundle source.",
				allow_source_change: options.allow_source_change,
			},
			state,
		);
	...
	function promptInteractiveWorkflowSelectionWithKeys(preview, options = {}) {
		return promptInteractiveSelectionWithKeys(
			...
			(state) =>
				renderInteractiveWorkflowSelectionKeyboardUi(preview, state, options),
			options,
		);
	}
	...
	function promptInteractiveSkillSelectionWithKeys(preview, options = {}) {
		return promptInteractiveSelectionWithKeys(
			...
			(state) =>
				renderInteractiveSkillSelectionKeyboardUi(preview, state, options),
			options,
		);
	}
	...
	function getSkillSourcePreviewer() {
		...
		const runtime = require(path.join(runtimeSrc, "lib", "runtime.cjs"));
		const skillRuntime = require(
			path.join(runtimeSrc, "lib", "skill-runtime.cjs"),
		);
		const runtimeConfig = runtime.loadRuntimeConfig(runtimeSrc);
		...
		const helper = skillRuntime.createSkillRuntimeHelpers({
			...
			getProjectExtDir: () =>
				runtime.getProjectExtDir(path.resolve(process.cwd())),
			updateSession() {},
	...
	function getWorkflowSourcePreviewer() {
		...
		const runtime = require(path.join(runtimeSrc, "lib", "runtime.cjs"));
		const workflowImportHelpers = require(
			path.join(runtimeSrc, "lib", "workflow-import.cjs"),
		);
		const workflowRegistry = require(
			path.join(runtimeSrc, "lib", "workflow-registry.cjs"),
		);
		...
		cachedWorkflowSourcePreviewer = (source, options = {}) => {
			...
			try {
				const sourceLayout = workflowImport.resolveWorkflowSourceLayout(
					staged.root,
					options,
				);
	...
	function resolveInstallerDefaultSkillSource() {
		const runtime = require(path.join(runtimeSrc, "lib", "runtime.cjs"));
		const runtimeConfig = runtime.loadRuntimeConfig(runtimeSrc);
		return defaultSkillSourceHelpers.resolveDefaultSkillSource(
			runtimeConfig,
			process.env,
		);
	}
	...
	function buildInstallerSkillSourceArgv(source) {
		...
		if (resolvedSource && resolvedSource === defaultSource.location) {
			return defaultSkillSourceHelpers.buildSkillSourceInstallArgv(
				defaultSource,
			);
		}
	...
	function previewInteractiveSkillSource(source, selectedSkillNames) {
		...
		const argv = [
			...buildInstallerSkillSourceArgv(source),
			"--scope",
			"project",
		];
		(Array.isArray(selectedSkillNames) ? selectedSkillNames : []).forEach(
			(name) => {
	...
	function formatInteractiveWorkflowSourceWarning(error) {
		...
		if (
			/Workflow registry source was not found or is not accessible\./i.test(
				message,
			)
		) {
		...
		if (
			/Could not reach workflow registry source\. Check your network connection and try again\./i.test(
				message,
			)
		) {
		...
		if (
			/required local dependency is not available in this environment/i.test(
				message,
			)
		) {
		...
		if (
			/Failed to clone workflow registry source|Could not download workflow registry source/i.test(
				message,
			)
		) {
		...
		return (
			message ||
			"Could not inspect that spec source. Enter another source or type `skip` to continue without external specs."
		);
	}
	...
	function formatInteractiveSkillSourceWarning(error) {
		...
		if (
			/Could not reach skill source\. Check your network connection and try again\./i.test(
				message,
			)
		) {
		...
		if (
			/git clone failed|npm install failed|pip install failed/i.test(message)
		) {
		...
		return (
			message ||
			"Could not inspect that skill source. Enter another source or type `skip` to continue without initial skills."
		);
	}
	...
	function writePromptConfirmation(label, value) {
		const { chalk } = createPromptStyler();
		writePromptOutput(
			`${chalk.green("✓")} ${chalk.cyan(label)} ${chalk.gray(value)}\n`,
		);
	}
	...
	async function promptInteractiveInstallArgs() {
		...
		if (targets.length === 0) {
			throw new Error(
				"No supported runtime targets are available for installation",
			);
		}
		...
		if (typeof promptInstallerChoices === "function") {
			...
			const runtime = String(
				prompted && prompted.runtime ? prompted.runtime : targets[0].name,
			)
				.trim()
				.toLowerCase();
			const location = String(
				prompted && prompted.location
					? prompted.location
					: getDefaultInstallLocation(runtime),
			)
				.trim()
				.toLowerCase();
			const developer = String(
				prompted && prompted.developer ? prompted.developer : "",
			).trim();
			const profile = normalizeInstallProfile(
				prompted && prompted.profile ? prompted.profile : "core",
				"",
			);
			const subagentBridgeCmd = String(
				prompted && prompted.subagentBridgeCmd
					? prompted.subagentBridgeCmd
					: "",
			).trim();
			...
			const subagentBridgeTimeoutProvided =
				prompted &&
				prompted.subagentBridgeTimeoutMs !== undefined &&
				prompted.subagentBridgeTimeoutMs !== null;
			if (!developer) {
				throw new Error("Interactive install requires developer name.");
			}
			if (!subagentBridgeCmd && subagentBridgeTimeoutProvided) {
				throw new Error(
					"Interactive sub-agent bridge timeout requires a bridge command.",
				);
			}
			...
			return {
				...
				runtime,
				developer,
				language: String(prompted && prompted.language ? prompted.language : "")
					.trim()
					.toLowerCase(),
				profile,
				interactive: true,
				configDir: "",
				subagentBridgeCmd,
				subagentBridgeTimeoutMs: subagentBridgeTimeoutProvided
					? parsePositiveInteger(
							prompted.subagentBridgeTimeoutMs,
		...
		const runtimeAnswer = await promptLine(
			buildInteractiveRuntimePrompt(targets),
		);
		...
		const locationAnswer = await promptLine(
			buildInteractiveLocationPrompt(target),
		);
		const resolvedLocationAnswer = String(
			locationAnswer || (defaultLocation === "local" ? "2" : "1"),
		).trim();
		const isLocal = resolvedLocationAnswer === "2";
		writePromptConfirmation(
			"Location:",
			isLocal ? "Local project" : "Global config",
		);
		...
		async function promptForWorkflowSourceOverride() {
			...
			while (!resolvedWorkflowPreview) {
				const workflowSourceInput = await promptLine(
					buildInteractiveWorkflowSourcePrompt(defaultWorkflowSource),
				);
				...
				const sourceConfig = buildInstallerWorkflowSourceConfig(
					normalizedInput,
					{},
				);
				try {
					resolvedWorkflowPreview = await previewInteractiveWorkflowSource(
		...
		if (isLocal) {
			...
			while (resolvedWorkflowPreview && !skipWorkflowImport) {
				if (
					...
					const selectedSpecs =
						await promptInteractiveWorkflowSelectionWithKeys(
				...
				try {
					...
					const normalizedSelectionInput = String(selectionInput || "")
						.trim()
						.toLowerCase();
					if (
						!normalizedSelectionInput ||
						normalizedSelectionInput === "skip"
					) {
						skipWorkflowImport = true;
						break;
					}
					...
					const selectedSpecs = parseInteractiveWorkflowSelection(
						selectionInput,
						resolvedWorkflowPreview,
					);
					if (selectedSpecs === null) {
		...
		async function promptForSkillSourceOverride() {
			while (!resolvedPreview) {
				const skillSourceInput = await promptLine(
					buildInteractiveSkillSourcePrompt(),
				);
				...
				try {
					resolvedPreview = await previewInteractiveSkillSource(
						skillSource,
						[],
					);
		...
		while (resolvedPreview && !skipInitialSkills) {
			if (supportsInteractiveSkillSelectionKeyboardUi()) {
				const selectedSkillNames =
					await promptInteractiveSkillSelectionWithKeys(resolvedPreview, {
						allow_source_change: true,
					});
			...
			try {
				...
				const selectedSkillNames = parseInteractiveSkillSelection(
					selectionInput,
					resolvedPreview,
				);
				if (selectedSkillNames === null) {
		...
		writePromptConfirmation(
			"Reply language:",
			language === "zh" ? "Chinese (Simplified)" : "English",
		);
	...
	function installRuntimeDependencies(runtimeDir) {
		const sourceNodeModulesDir = path.join(
			path.resolve(runtimeSrc, ".."),
			"node_modules",
		);
		if (!fs.existsSync(sourceNodeModulesDir)) {
			throw new Error(
				`Runtime dependencies are not installed under: ${sourceNodeModulesDir}`,
			);
		}
	...
	function copyFileWithReplacement(
		sourcePath,
		targetPath,
		installRoot,
		target,
	...
	function getRuntimeTarget(args) {
		...
		if (!target.supported) {
			throw new Error(
				`Runtime target "${target.name}" is not supported yet: ${target.notSupportedReason}`,
			);
		}
	...
	function replaceInstallPaths(content, targetDir, target) {
		...
		for (const pattern of installTargets.getManagedRuntimePathPatterns(
			target,
		)) {
	...
	function renderRuntimeTemplate(
		templateName,
		replacements,
		targetDir,
		target,
		...
		let content = replaceInstallPaths(
	...
	function buildConfigBlock(targetDir, target, agents) {
		const agentsDir = path
			.join(targetDir, target.agentsDirName || "agents")
			.replace(/\\/g, "/");
		const lines = [];
	...
	function buildRustResolverInvocation(runtimeDir) {
		...
		const overrideCommand = String(
			process.env.EMB_AGENT_RUST_HOOK_CMD || "",
		).trim();
		...
		if (
			path.basename(path.resolve(runtimeDir)) === "runtime" &&
	...
	function buildFallbackHookPlan(
		runtimeDir,
		target,
		hookName,
		hookFileName,
	...
	function buildResolvedHookPlan(runtimeDir, target, hookFileName) {
		const hookName = normalizeHookNameFromFile(hookFileName);
		const fallback = buildFallbackHookPlan(
			runtimeDir,
			target,
			hookName,
			hookFileName,
		...
		try {
			const result = require("child_process").spawnSync(
				...
				[
					...invocation.args,
					"--host",
					target && target.name ? target.name : "external",
					"--hook",
					hookName,
					"--runtime-dir",
					runtimeDir,
			...
			return {
				...
				host: String(
					parsed.host || (target && target.name ? target.name : "external"),
				),
	...
	function ensureJsonHostHooks(settingsPath, targetDir, target, isLocal) {
		...
		const sessionStartCommand = buildJsonHostHookCommand(
			targetDir,
			target,
			"emb-session-start.js",
			isLocal,
		);
		const contextMonitorCommand = buildJsonHostHookCommand(
			targetDir,
			target,
			"emb-context-monitor.js",
			isLocal,
		);
		const statusLineCommand = buildJsonHostHookCommand(
			targetDir,
			target,
			"emb-statusline.js",
			isLocal,
		...
		const hasSessionStartHook = next.hooks.SessionStart.some(
			(entry) =>
				entry &&
					...
					(Array.isArray(entry.hooks) &&
						entry.hooks.some(
							(hook) =>
								...
								commandMatchesManagedHook(hook.command, "emb-session-start.js"),
						))),
		);
		...
		const hasContextMonitorHook = next.hooks.PostToolUse.some(
			(entry) =>
				entry &&
				((typeof entry.command === "string" &&
	...
	function stripJsonHostManagedHooks(settings) {
		...
		for (const eventName of ["SessionStart", "PostToolUse"]) {
			const entries = Array.isArray(next.hooks[eventName])
	...
	function installCodexAgents(targetDir, target) {
		...
		const configPath = path.join(
			targetDir,
			target.configFileName || "config.toml",
		);
		mergeManagedConfig(
			configPath,
	...
	function buildDefaultHostCommandInvocation(runtimeCli, commandName) {
		...
		return command === "start" || command === "next" ? `${base} --brief` : base;
	}
	...
	function buildHostCommandUxLines(commandName, runtimeCli) {
		...
		if (command === "start" || command === "next") {
			lines.push(
				`- Use \`${buildDefaultHostCommandInvocation(runtimeCli, command)}\` by default so the host sees compact routing JSON; only run full output when the user explicitly asks for it.`,
			);
		}

		lines.push(
			"- Treat emb-agent output as a machine protocol for the AI host, not as human-facing text. Use `agent_protocol` as the primary routing contract.",
		);
		lines.push(
			"- Do not paste raw JSON or a full command transcript into chat. Summarize status, blockers, and the recommended action in plain language.",
		);
		lines.push(
		...
		lines.push(
			"- If `agent_protocol.gate.kind` is `execution`, treat the payload as an execution brief: perform the requested repository change now, then verify after implementation evidence exists.",
		);
		lines.push(
			"- If the user request embeds an unconfirmed technical choice, route through `decision review` / `decision record` before implementation instead of letting the model silently validate the premise.",
		);
		lines.push(
			"- Do not automatically execute `agent_protocol.recommendation.command`; stop after the recommendation unless the user explicitly asks to continue. If a legacy payload lacks `agent_protocol`, fall back to compact `next`/`immediate` fields only.",
		);
		lines.push(
			"- When showing a follow-up action, prefer the short emb-agent command form (for example `capability run scan` or `task add <summary>`) over the full `node .../emb-agent.cjs ...` path unless the user needs a copy-paste CLI.",
		);
	...
	function installCodexHooks(targetDir, target, args) {
		const hooksPath = path.join(
			targetDir,
			target.hooksConfigFileName || "hooks.json",
		);
		...
		for (const [eventName, entries] of Object.entries(rendered.hooks || {})) {
			const existingEntries = Array.isArray(next.hooks[eventName])
				? next.hooks[eventName]
				: [];
	...
	function generateClaudeCommandContent(commandName, content, runtimeDir) {
		const { frontmatter, body } = extractFrontmatterAndBody(content);
		const commandLabel =
			extractFrontmatterField(frontmatter, "name") || `emb-${commandName}`;
		const description = toSingleLine(
			extractFrontmatterField(frontmatter, "description") ||
				`Run emb-agent ${commandName}`,
		);
		const runtimeCli = runtimeHost.resolveRuntimeHost(runtimeDir).cliCommand;
		const defaultInvocation = buildDefaultHostCommandInvocation(
			runtimeCli,
			commandName,
		);
	...
	function installPiExtension(targetDir, target, runtimeDir) {
		...
		const runtimeCli = runtimeHost.resolveRuntimeHost(runtimeDir).cliCommand;
		const hookPath = (hookFileName) =>
			path.join(runtimeDir, "hooks", hookFileName).replace(/\\/g, "/");
		const rendered = renderRuntimeTemplate(
			"pi-extension.ts.tpl",
			{
				RUNTIME_CLI_JSON: JSON.stringify(runtimeCli),
				RUNTIME_CLI_PATH_JSON: JSON.stringify(
					runtimeHost
						.resolveRuntimeHost(runtimeDir)
						.cliPath.replace(/\\/g, "/"),
				),
				SESSION_START_HOOK_JSON: JSON.stringify(
					hookPath("emb-session-start.js"),
				),
				CONTEXT_MONITOR_HOOK_JSON: JSON.stringify(
					hookPath("emb-context-monitor.js"),
				),
				STATUSLINE_HOOK_JSON: JSON.stringify(hookPath("emb-statusline.js")),
				HOOK_RUNTIME_JSON: JSON.stringify({
					session_start: buildResolvedHookPlan(
						runtimeDir,
						target,
						"emb-session-start.js",
					),
					statusline: buildResolvedHookPlan(
						runtimeDir,
						target,
						"emb-statusline.js",
					),
					context_monitor: buildResolvedHookPlan(
						runtimeDir,
						target,
						"emb-context-monitor.js",
					),
		...
		fs.writeFileSync(
			path.join(extensionRoot, "emb-agent.ts"),
			rendered,
			"utf8",
		);
		return 1;
	...
	function generateCodexSkillContent(commandName, content, runtimeDir) {
		...
		const conversationGuardrails =
			buildCodexConversationGuardrails(commandName);
		...
		return [
			...
			...(conversationGuardrails.length > 0
				? ["", "## Conversation UX", "", ...conversationGuardrails]
				: []),
			"",
			"## Original Guidance",
			"",
	...
	function installAgents(targetDir, target, args) {
		...
		if (
			target.agentMode === "markdown" ||
			target.hookMode === "claude-settings"
		) {
	...
	function installRuntime(targetDir, target, args) {
		...
		for (const file of fs.readdirSync(runtimeHooksSrc)) {
			...
			if (fs.statSync(sourcePath).isDirectory()) {
				copyDirWithReplacement(
					sourcePath,
					path.join(runtimeHooksDir, file),
					targetDir,
					target,
				);
				continue;
			...
			fs.writeFileSync(
				path.join(runtimeHooksDir, file),
				replaceInstallPaths(raw, targetDir, target),
			);
		}
		copyDirWithReplacement(
			path.join(runtimeSrc, "lib"),
			path.join(runtimeDir, "lib"),
			targetDir,
			target,
		);
		copyDirWithReplacement(
			path.join(runtimeSrc, "scripts"),
		...
		if (installProfile.includeScaffolds) {
			copyDirWithReplacement(
				path.join(runtimeSrc, "scaffolds"),
				path.join(runtimeDir, "scaffolds"),
				targetDir,
				target,
			);
		}
		copyDirWithReplacement(
			path.join(runtimeSrc, "templates"),
			path.join(runtimeDir, "templates"),
			targetDir,
			target,
		);
		copyDir(
			path.join(runtimeSrc, "registry"),
			path.join(runtimeDir, "registry"),
		);
		copyDir(
			path.join(runtimeSrc, "profiles"),
			path.join(runtimeDir, "profiles"),
		);
		copyDir(
			path.join(path.resolve(runtimeSrc, ".."), "skills"),
		...
		fs.copyFileSync(
			path.join(runtimeSrc, "config.json"),
		...
		fs.writeFileSync(
			...
			JSON.stringify(
				runtimeHost.createInstallHostMetadata(targetDir, target, args),
				null,
				2,
			) + "\n",
			"utf8",
		...
		for (const file of fs
			...
			const commandName = file.replace(/\.md$/, "");
			fs.writeFileSync(
				path.join(runtimeCommandDocsDir, file),
				rendered,
				"utf8",
			);
			if (commandVisibility.isPublicCommandName(commandName)) {
		...
		for (const file of fs
			.readdirSync(agentsSrc)
			.filter(
				(name) => name.startsWith(AGENT_PREFIX) && name.endsWith(".md"),
			)) {
			const targetName = file.replace(/^emb-/, "");
			const raw = fs.readFileSync(path.join(agentsSrc, file), "utf8");
			fs.writeFileSync(
				path.join(runtimeAgentsDir, targetName),
				replaceInstallPaths(raw, targetDir, target),
				"utf8",
			);
		}

		// Copy Rust binary if available (postinstall download or source build)
		const pkgRoot = path.resolve(runtimeSrc, "..");
		const exeName =
			process.platform === "win32" ? "emb-agent-rs.exe" : "emb-agent-rs";
		const candidates = [
			path.join(pkgRoot, "bin", exeName),
			path.join(pkgRoot, "target", "debug", exeName),
			path.join(pkgRoot, "target", "release", exeName),
		];
		for (const candidate of candidates) {
			if (fs.existsSync(candidate)) {
				...
				try {
					fs.chmodSync(path.join(destBinDir, exeName), 0o755);
				} catch {
					/* non-fatal */
				}
				break;
	...
	function createInstallReporter(args) {
		...
		function announce(target, args, targetDir, installProfile) {
			...
			writeTerminalLine(
				drawBoxLine(bottomLeft, fill, bottomRight, contentWidth),
			);
			writeTerminalLine("");
			writeTerminalLine(
				`${chalk.cyan("  Runtime:")}  ${chalk.white(target.label)}`,
			);
			writeTerminalLine(
				`${chalk.cyan("  Location:")} ${chalk.white(args.local ? "project (local)" : "global config")}`,
			);
			writeTerminalLine(`${chalk.cyan("  Target:")}   ${chalk.dim(targetDir)}`);
			writeTerminalLine(
				`${chalk.cyan("  Profile:")}  ${chalk.white(installProfile.name)}`,
			);
			if (args.developer) {
				writeTerminalLine(
					`${chalk.cyan("  Developer:")} ${chalk.white(args.developer)}`,
				);
			}
		...
		function complete(
			...
			writeTerminalLine(
				`${chalk.green("  ●")} ${chalk.white("Runtime")}        ${chalk.dim(runtimeDir)}`,
			);
			if (target.managesHostConfig === false) {
				if (target.name === "pi") {
					writeTerminalLine(
						`${chalk.green("  ●")} ${chalk.white("Pi extension")}   ${chalk.dim(path.join(targetDir, "extensions", "emb-agent.ts"))}`,
					);
				} else {
					writeTerminalLine(
						`${chalk.green("  ●")} ${chalk.white("Host metadata")}  ${chalk.dim(path.join(runtimeDir, "HOST.json"))}`,
					);
				}
			} else {
				writeTerminalLine(
					`${chalk.green("  ●")} ${chalk.white("Host config")}     ${chalk.dim(path.join(targetDir, target.configFileName || "config.toml"))}`,
				);
			}
			if (target.hookMode === "codex-json") {
				writeTerminalLine(
					`${chalk.green("  ●")} ${chalk.white("Hooks")}          ${chalk.dim(path.join(targetDir, target.hooksConfigFileName || "hooks.json"))}`,
				);
			}
			if (
				projectBootstrap &&
				projectBootstrap.bootstrap_task &&
				projectBootstrap.bootstrap_task.path
			) {
				writeTerminalLine(
					`${chalk.green("  ●")} ${chalk.white("Project")}        ${chalk.dim(projectBootstrap.project_root)}`,
				);
			}
			writeTerminalLine("");
			writeTerminalLine(chalk.dim("  ── Next steps ──"));
			writeTerminalLine(
				`  ${chalk.cyan("1.")} Restart ${chalk.white(target.restartLabel || target.label)} to pick up new commands, hooks, or extensions`,
			);
			writeTerminalLine(
				`  ${chalk.cyan("2.")} Open a project session — startup context injects automatically`,
			);
			if (projectBootstrap) {
				writeTerminalLine(
					`  ${chalk.cyan("3.")} Follow the recommended next command from the injected context`,
				);
			}
			writeTerminalLine("");
		}
	...
	function uninstall(targetDir, target, args) {
		...
		if (target.managesHostConfig !== false) {
			...
			if (fs.existsSync(configPath)) {
				if (
					target.hookMode === "claude-settings" ||
					target.hookMode === "cursor-settings"
				) {
					const cleanedSettings = stripJsonHostManagedHooks(
					...
					const cleaned = stripManagedConfigBlock(
		...
		if (target.hookMode === "codex-json") {
			...
			if (fs.existsSync(hooksConfigPath)) {
				const cleanedHooks = stripJsonHostManagedHooks(
					readJsonObject(hooksConfigPath),
				);
				if (
					cleanedHooks &&
	...
	async function installInitialSkills(runtimeDir, args) {
		const sources = Array.isArray(args && args.skillSources)
			? args.skillSources.filter(Boolean)
			: [];
		if (sources.length === 0) {
			return [];
		}

		const installedRuntime = require(
			path.join(runtimeDir, "lib", "emb-agent-main.cjs"),
		);
		const sharedArgs = [
			"--scope",
			args && args.local ? "project" : "user",
			...(args && args.force ? ["--force"] : []),
			...(args && Array.isArray(args.skillNames)
				? args.skillNames
				: []
			).flatMap((name) => ["--skill", name]),
		];

		return Promise.all(
			sources.map((source) =>
				installedRuntime.installSkillSource([
					...buildInstallerSkillSourceArgv(source),
					...sharedArgs,
				]),
			),
	...
	async function main(argv) {
		...
		if (args.uninstall) {
			...
			process.stdout.write(
				`Uninstalled emb-agent managed files for ${target.label} from: ${targetDir}\n`,
			);
			return;
		}
		...
		const integrationActivity = reporter.activity(
		...
		try {
			...
			sharedSkillCount = installSharedCodexSkills(
				targetDir,
				target,
				runtimeDir,
				args,
		...
		const envExamplePath = path.join(
			args.local ? process.cwd() : targetDir,
			".env.example",
		);
		...
		try {
			...
			envActivity.succeed(
				envExampleCreated ? ".env.example created" : ".env.example kept",
			);
		} catch (error) {
		...
		if (args.local) {
			const bootstrapActivity = reporter.activity(
				"Bootstrapping project scaffold (.emb-agent/)",
			);
			try {
				...
				bootstrapActivity.succeed(
					projectBootstrap
						...
						: "Project scaffold skipped (already exists)",
				);
			} catch (error) {
		...
		if (Array.isArray(args.skillSources) && args.skillSources.length > 0) {
			...
			try {
				...
				const alreadyInstalled = results.filter(
					(r) => r.status === "already_installed",
				);
				const updatesAvailable = results.filter(
				...
				if (newCount > 0) {
					parts.push(
						`${newCount} skill bundle${newCount > 1 ? "s" : ""} installed`,
					);
				}
				if (skipCount > 0) {
				...
				if (updateCount > 0) {
					...
					parts.push(
						`update available: ${labels.join(", ")} (use --force to update)`,
					);
				}
		...
		const lines = [
			...
			...(installedRuntimeHost.subagentBridge &&
			installedRuntimeHost.subagentBridge.available
				? [
			...
			...installedSkillBundles.map((bundle) => {
				const enabledCount = Array.isArray(bundle.selected_skills)
					? bundle.selected_skills.length
					: 0;
