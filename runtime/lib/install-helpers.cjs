...
function createInstallHelpers(deps) {
  ...
  function buildHostCommandUxLines(commandName, runtimeCli) {
    ...
    lines.push('- Respect `delegation_policy` / `agent_protocol.gate.delegation_policy`: when the host exposes subagents and work is broad (system framework, multiple peripherals, power/sleep/watchdog/LVD/config bits, toolchain/SDK integration, or implementation plus review), list available subagents first and delegate scouts/reviewers or focused workers instead of doing everything inline.');
    lines.push('- During `prd-exploration`, subagent delegation is allowed only for read-only evidence scouts/reviewers; do not dispatch implementation workers until a concrete task is active.');
