...
function readHeader(fileBuffer) {
	if (!Buffer.isBuffer(fileBuffer) || fileBuffer.length < 512) {
		throw new Error(
			"SchDoc file is too small to contain a valid compound file header",
		);
	}
...
function readChain(
	...
	if (
		startSector === CFB_SPECIAL.endOfChain ||
		startSector === CFB_SPECIAL.free
	) {
	...
	while (
		currentSector !== CFB_SPECIAL.endOfChain &&
		currentSector !== CFB_SPECIAL.free
	) {
		if (seen.has(currentSector)) {
			throw new Error(
				`CFB sector chain loop detected at sector ${currentSector}`,
			);
		}
	...
	return typeof expectedSize === "number"
		? data.subarray(0, expectedSize)
		: data;
}
...
function parseDirectoryEntries(fileBuffer, header, fatEntries) {
	const directoryData = readChain(
		...
		header.numDirSectors > 0
			? header.numDirSectors * header.sectorSize
			: undefined,
	);
	const entries = [];
...
function readMiniStream(
	...
	while (
		...
		if (seen.has(currentSector)) {
			throw new Error(
				`CFB mini-sector chain loop detected at sector ${currentSector}`,
			);
		}
		if (currentSector >= miniFatEntries.length) {
			throw new Error(
				`CFB mini-sector ${currentSector} is outside mini FAT range`,
			);
		}
...
function readNamedStream(fileBuffer, streamName) {
	...
	const entry = directoryEntries.find(
		(item) => item.type === 2 && item.name === streamName,
	);
	...
	if (entry.size < header.miniStreamCutoff) {
		if (!rootEntry) {
			throw new Error(
				"CFB root entry is missing; mini stream cannot be resolved",
			);
		}
...
function splitRecordLines(streamBuffer) {
	const payload = streamBuffer.subarray(
		5,
		Math.max(5, streamBuffer.length - 1),
	);
	const lines = [];
	let start = 0;
	...
	for (let index = 0; index <= payload.length - 6; index += 1) {
		if (
			payload[index + 3] === 0x00 &&
			payload[index + 4] === 0x00 &&
			payload[index + 5] === 0x7c
		) {
...
function parseSchDocRecords(fileBuffer) {
	...
	const header = datums.filter((item) => Object.hasOwn(item, "HEADER"));
	const records = datums.filter((item) => Object.hasOwn(item, "RECORD"));
...
function parseCoord(record, key) {
	...
	const fracPart = parseNumberField(
		record,
		0,
		`${key}_Frac`,
		`${key.toUpperCase()}_FRAC`,
	);
...
function parsePointList(record) {
	const points = [];
	const locationCount = parseNumberField(
		record,
		0,
		"LocationCount",
		"LOCATIONCOUNT",
...
function findNamedText(records, targetName) {
	...
	const record = records.find((item) => {
		...
		return (
			(item.RECORD === "41" || item.RECORD === "34") &&
			recordName === lowerTarget
		);
	});
...
function normalizeComponentInput(partRecord, relatedRecords) {
	...
	const preferCommentAsValue =
		/^(sop-?\d+|soic-?\d+|qfn-?\d+|ssop-?\d+|tssop-?\d+|dip-?\d+|qfp-?\d+|r|c|d|led|testpoint)$/i.test(
			String(rawValue || ""),
	...
	relatedRecords
		...
		.forEach((item) => {
			...
			if (
				name &&
				text &&
				!["Designator", "Comment", "Value", "Footprint", "Datasheet"].includes(
					name,
				)
			) {
...
function buildDevice(record, partMap) {
	if (record.RECORD === "2") {
		const rotation =
			(parseInt(
				getField(record, "PinConglomerate", "PINCONGLOMERATE") || "0",
				10,
			) &
				0x03) *
			90;
		const pinLength = parseInt(
			getField(record, "PinLength", "PINLENGTH") || "0",
			10,
		);
		const locationX = parseInt(
			getField(record, "Location.X", "LOCATION.X") || "0",
			10,
		);
		const locationY = parseInt(
			getField(record, "Location.Y", "LOCATION.Y") || "0",
			10,
		);
	...
	if (record.RECORD === "27") {
		...
		const locationCount = parseInt(
			getField(record, "LocationCount") || "0",
			10,
		);
		for (let index = 1; index <= locationCount; index += 1) {
...
function getDeviceNetText(device) {
	const record = device || {};
	// Ports (RECORD=18) store their name in the Name/NAME field, not Text/TEXT
	if (record.RECORD === "18") {
		return getField(record, "Name", "NAME", "%UTF8%Text", "Text", "TEXT");
	}
	return getField(record, "%UTF8%Text", "Text", "TEXT");
}
...
function checkPortPinProximity(port, pin) {
	// Only check port-to-pin proximity
	if (port.RECORD !== "18" || pin.RECORD !== "2") {
		return false;
	}

	const portCoords = Array.isArray(port.coords) ? port.coords[0] : null;
	const pinCoords = Array.isArray(pin.coords) ? pin.coords[0] : null;
	if (!portCoords || !pinCoords) {
		return false;
	}

	const portX = Number(portCoords[0]);
	const portY = Number(portCoords[1]);
	const pinX = Number(pinCoords[0]);
	const pinY = Number(pinCoords[1]);

	// Port Location is the text anchor in Altium.
	// Alignment 1 = text left, body extends right from Location.X
	// Alignment 2 = text right, body extends left from Location.X
	// Alignment 3 = centered on Location
	const alignment = Number(getField(port, "Alignment", "ALIGNMENT") || "1");
	const width = Math.max(Number(getField(port, "Width", "WIDTH") || "46"), 10);
	const height = Math.max(
		Number(getField(port, "Height", "HEIGHT") || "10"),
		6,
	);
	const tolerance = 5; // schematic grid tolerance (mils)

	let bodyLeft;
	let bodyRight;

	if (alignment === 2) {
		// Body extends left from anchor
		bodyLeft = portX - width;
		bodyRight = portX;
	} else if (alignment === 3) {
		// Centered
		bodyLeft = portX - width / 2;
		bodyRight = portX + width / 2;
	} else {
		// Alignment 1 (default): body extends right from anchor
		bodyLeft = portX;
		bodyRight = portX + width;
	}

	const bodyTop = portY - height / 2;
	const bodyBottom = portY + height / 2;

	// Check if pin endpoint falls within the port body (plus tolerance)
	return (
		pinX >= bodyLeft - tolerance &&
		pinX <= bodyRight + tolerance &&
		pinY >= bodyTop - tolerance &&
		pinY <= bodyBottom + tolerance
	);
}
...
function devicesConnected(a, b) {
	...
	for (const [start, end] of aSegments) {
		if (
			pointOnSegment(start, start, end) &&
			bSegments.some(([otherStart, otherEnd]) =>
				pointOnSegment(start, otherStart, otherEnd),
			)
		) {
			return true;
		}
		if (
			pointOnSegment(end, start, end) &&
			bSegments.some(([otherStart, otherEnd]) =>
				pointOnSegment(end, otherStart, otherEnd),
			)
		) {
			return true;
		}
	...
	for (const [start, end] of bSegments) {
		if (
			pointOnSegment(start, start, end) &&
			aSegments.some(([otherStart, otherEnd]) =>
				pointOnSegment(start, otherStart, otherEnd),
			)
		) {
			return true;
	...
	// Port-to-pin proximity: ports have bounding boxes (not single points).
	// Altium port Location is the text anchor; the body rectangle extends
	// based on Alignment. Pins touching or inside the port body connect.
	if (checkPortPinProximity(a, b) || checkPortPinProximity(b, a)) {
		return true;
	}
	...
	if (
		...
		bText &&
		aText === bText &&
		["17", "18", "25"].includes(a.RECORD) &&
		["17", "18", "25"].includes(b.RECORD)
	) {
		return true;
	}
...
function buildNets(records, partMap) {
	const devices = records
		.filter((record) =>
			["2", "17", "18", "25", "27", "29"].includes(record.RECORD),
		)
	...
	devices.forEach((device) => {
		...
		while (stack.length > 0) {
			...
			devices.forEach((candidate) => {
				if (
					!visited.has(candidate.index) &&
					devicesConnected(current, candidate)
				) {
		...
		const namedDevice = group.find((item) =>
			["17", "18", "25"].includes(item.RECORD),
		);
		const name =
			getDeviceNetText(namedDevice) || `UNNAMED_NET_${nets.length + 1}`;
		...
		nets.push({
			...
			members,
			evidence,
			confidence: name.startsWith("UNNAMED_NET_")
				? "heuristic-unnamed"
				: "heuristic-named",
		});
	});
...
function isPassiveTwoTerminal(component) {
	const libref = String(component?.libref || "").toUpperCase();
	// Common passive 2-terminal component library references
	return (
		libref === "R" ||
		libref === "R_US" ||
		libref === "C" ||
...
function isPowerRail(name) {
	...
	return (
		...
		lower === "1V8" ||
		lower === "1.8V" ||
		lower.startsWith("UNNAMED_NET_")
	);
}

function propagateNetNamesThroughPassives(nets, partMap) {
	// Build pin -> net index
	const pinToNet = new Map();
	nets.forEach((net) => {
		net.members.forEach((member) => {
			pinToNet.set(member, net.name);
		});
	});

	// Build designator -> component lookup
	const componentByDesignator = new Map();
	partMap.forEach((component) => {
		if (component.designator) {
			componentByDesignator.set(component.designator, component);
		}
	});
	...
	while (changed && iterations < maxIterations) {
		...
		nets.forEach((net) => {
			// Only propagate from named (non-power) nets
			if (isPowerRail(net.name)) {
				return;
			}

			net.members.forEach((member) => {
				const [designator, pinNum] = String(member).split(".");
				const component = componentByDesignator.get(designator);
				if (!component || !isPassiveTwoTerminal(component)) {
					return;
				}

				// Find the other pin of this passive component
				const otherPin = component.pins?.find(
					(p) => String(p.number) !== String(pinNum),
				);
				if (!otherPin) {
					return;
				}

				const otherMember = `${designator}.${otherPin.number}`;
				const otherNetName = pinToNet.get(otherMember);

				if (
					otherNetName &&
					otherNetName !== net.name &&
					otherNetName.startsWith("UNNAMED_NET_") &&
					!otherNetName.startsWith("sheet-1:")
				) {
					// Rename the other net and update the index
					const otherNet = nets.find((n) => n.name === otherNetName);
					if (otherNet) {
						otherNet.name = net.name;
						otherNet.confidence = "heuristic-named";
						// Update pinToNet for all members of the renamed net
						otherNet.members.forEach((m) => {
							pinToNet.set(m, net.name);
						});
						changed = true;
					}
				}
	...
	nets.forEach((net) => {
		if (merged.has(net.name)) {
			const existing = merged.get(net.name);
			existing.members = runtimeLikeUnique([
				...existing.members,
				...net.members,
			]);
			existing.evidence = runtimeLikeUnique([
				...existing.evidence,
				...net.evidence,
			]);
			existing.confidence =
				existing.confidence === "heuristic-named" ||
				net.confidence === "heuristic-named"
					? "heuristic-named"
					: "heuristic-unnamed";
		} else {
			merged.set(net.name, {
...
function buildBom(components) {
	...
	return Array.from(groups.values())
		.map((item) => ({
			...item,
			designators: item.designators.sort((a, b) =>
				String(a).localeCompare(String(b), undefined, { numeric: true }),
			),
		}))
		.sort((a, b) =>
			String(a.designators[0] || "").localeCompare(
				String(b.designators[0] || ""),
				undefined,
				{ numeric: true },
			),
		);
...
function previewPinBodyPoint(pin) {
	if (
		Number.isFinite(Number(pin.body_x)) &&
		Number.isFinite(Number(pin.body_y))
	) {
...
function previewPinCandidates(record, partMap) {
	...
	const orientation =
		parseNumberField(record, 0, "PinConglomerate", "PINCONGLOMERATE") & 0x03;
...
function previewPinConnectionScore(point, segments, connectionPoints) {
	...
	segments.forEach((segment) => {
		best = Math.min(
			best,
			previewPointDistanceToSegment(point, segment.a, segment.b),
		);
	});
	connectionPoints.forEach((candidate) => {
		best = Math.min(
			best,
			Math.hypot(
				Number(point.x) - Number(candidate.x),
				Number(point.y) - Number(candidate.y),
			),
		);
	});
	return best;
...
function reconcilePreviewPins(
	...
	return pinDrafts.map((draft) => {
		...
		return {
			...
			endpoint_score: Number.isFinite(selected.score)
...
function buildPreviewPrimitive(record) {
	...
	if (record.RECORD === "6") {
		...
		return points.length >= 2
			? { kind: "polyline", points, record_index: record.index }
			: null;
	}
	if (record.RECORD === "14") {
		return {
	...
	if (record.RECORD === "8") {
		...
		const secondaryRadius =
			getField(record, "SecondaryRadius", "SECONDARYRADIUS") === ""
				? radius
...
function buildPreviewInput(records, components, partMap) {
	...
	records.forEach((record) => {
		if (
			record.RECORD === "31" &&
			truthyFlag(getField(record, "UseCustomSheet", "USECUSTOMSHEET"))
		) {
			sheetSize = {
		...
		if (primitive) {
			...
			if (primitive.kind === "line")
				lines.push({ a: primitive.a, b: primitive.b });
			if (primitive.kind === "polyline") polylines.push(primitive.points);
			if (primitive.kind === "rectangle")
				rectangles.push({ a: primitive.a, b: primitive.b });
			if (primitive.kind === "roundRect")
				roundRects.push({
					a: primitive.a,
					b: primitive.b,
					rx: primitive.rx,
			...
			if (primitive.kind === "polygon")
				polygons.push({ points: primitive.points, filled: primitive.filled });
			if (primitive.kind === "bezier")
				beziers.push({ points: primitive.points });
		}
		...
		if (record.RECORD === "25") {
			...
			netLabels.push({
				...
				justification: parseNumberField(
					record,
					0,
					"Justification",
					"JUSTIFICATION",
		...
		if (record.RECORD === "18") {
			...
			ports.push({
				x: point.x,
				y: point.y,
				text: getField(record, "Name", "NAME", "%UTF8%Text", "Text", "TEXT"),
				ioType: parseNumberField(record, 0, "IOType", "IOTYPE"),
				height: parseNumberField(record, 10, "Height", "HEIGHT"),
				width: parseNumberField(record, 46, "Width", "WIDTH"),
				alignment: parseNumberField(record, 1, "Alignment", "ALIGNMENT"),
			});
		}
		if (
			record.RECORD === "4" ||
			record.RECORD === "34" ||
			(record.RECORD === "41" &&
				!truthyFlag(getField(record, "IsHidden", "ISHIDDEN")))
		) {
	...
	const pins = reconcilePreviewPins(
		pinDrafts,
		wires,
		busses,
		junctions,
	...
	(components || []).forEach((component) => {
		const primitives =
			componentPrimitives.get(String(component.raw_part_index)) || [];
...
function svgBoundsOf(input) {
	...
	(input.wires || []).forEach((item) => {
		add(item.a);
		add(item.b);
	});
	(input.busses || []).forEach((item) => {
		add(item.a);
		add(item.b);
	});
	(input.busEntries || []).forEach((item) => {
		add(item.a);
		add(item.b);
	});
	...
	(input.rectangles || []).forEach((item) => {
		add(item.a);
		add(item.b);
	});
	(input.roundRects || []).forEach((item) => {
		add(item.a);
		add(item.b);
	});
	...
	(input.ports || []).forEach((item) => {
		const hw = (item.width || 46) / 2;
		const hh = (item.height || 10) / 2;
		add({ x: item.x - hw, y: item.y - hh });
		add({ x: item.x + hw, y: item.y + hh });
	});
	if (input.sheetSize) {
		add({ x: 0, y: 0 });
...
function svgBaselineForJustification(justification) {
	const normalized = Number(justification || 0);
	if (normalized === 7 || normalized === 8 || normalized === 9)
		return "hanging";
	if (normalized === 4 || normalized === 5 || normalized === 6)
		return "central";
...
function buildPreviewSvg(input) {
	...
	parts.push(
		`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${Math.ceil(width * 4)}" height="${Math.ceil(height * 4)}" preserveAspectRatio="xMidYMid meet">`,
	);
	parts.push(`<style>
	...
	parts.push(
		`<rect class="preview-sheet-bg" x="0" y="0" width="${width}" height="${height}"/>`,
	);
	if (preview.sheetSize) {
		parts.push(
			`<rect class="preview-sheet" x="${tx(0)}" y="${ty(preview.sheetSize.y)}" width="${preview.sheetSize.x}" height="${preview.sheetSize.y}"/>`,
		);
	}
	(preview.components || []).forEach((component) => {
		parts.push(
			`<rect class="preview-component-body" data-ref-kind="component" data-ref-id="${component.record_index}" data-designator="${svgEscape(component.designator)}" x="${tx(component.x)}" y="${ty(component.y + component.h)}" width="${component.w}" height="${component.h}" rx="2" ry="2"/>`,
		);
	});
	(preview.wires || []).forEach((item) => {
		parts.push(
			`<line class="preview-wire" x1="${tx(item.a.x)}" y1="${ty(item.a.y)}" x2="${tx(item.b.x)}" y2="${ty(item.b.y)}"/>`,
		);
	});
	(preview.busses || []).forEach((item) => {
		parts.push(
			`<line class="preview-bus" x1="${tx(item.a.x)}" y1="${ty(item.a.y)}" x2="${tx(item.b.x)}" y2="${ty(item.b.y)}"/>`,
		);
	});
	(preview.busEntries || []).forEach((item) => {
		parts.push(
			`<line class="preview-bus-entry" x1="${tx(item.a.x)}" y1="${ty(item.a.y)}" x2="${tx(item.b.x)}" y2="${ty(item.b.y)}"/>`,
		);
	});
	(preview.lines || []).forEach((item) => {
		parts.push(
			`<line class="preview-line" x1="${tx(item.a.x)}" y1="${ty(item.a.y)}" x2="${tx(item.b.x)}" y2="${ty(item.b.y)}"/>`,
		);
	});
	(preview.polylines || []).forEach((polyline) => {
		parts.push(
			`<polyline class="preview-line" points="${polyline.map((point) => `${tx(point.x)},${ty(point.y)}`).join(" ")}" stroke-linejoin="miter"/>`,
		);
	});
	...
	(preview.roundRects || []).forEach((rectangle) => {
		...
		const maxY = Math.max(rectangle.a.y, rectangle.b.y);
		parts.push(
			`<rect class="preview-box" x="${tx(minX)}" y="${ty(maxY)}" width="${maxX - minX}" height="${maxY - minY}" rx="${rectangle.rx}" ry="${rectangle.ry}"/>`,
		);
	});
	(preview.arcs || []).forEach((arc) => {
		if (arcSweepDegrees(arc.startAngle, arc.endAngle) >= 359.9) {
			parts.push(
				`<circle class="preview-box" cx="${tx(arc.center.x)}" cy="${ty(arc.center.y)}" r="${arc.radius}"/>`,
			);
		} else {
			parts.push(
				`<path class="preview-line" d="${arcPathD(tx, ty, arc.center, arc.radius, arc.startAngle, arc.endAngle)}" fill="none"/>`,
			);
		}
	});
	(preview.ellipses || []).forEach((ellipse) => {
		parts.push(
			`<ellipse class="preview-box" cx="${tx(ellipse.center.x)}" cy="${ty(ellipse.center.y)}" rx="${ellipse.rx}" ry="${ellipse.ry}"/>`,
		);
	});
	(preview.polygons || []).forEach((polygon) => {
		const points = (polygon.points || [])
			.map((point) => `${tx(point.x)},${ty(point.y)}`)
			.join(" ");
		parts.push(
			`<polygon class="${polygon.filled ? "preview-fill" : "preview-box"}" points="${points}"/>`,
		);
	});
	(preview.beziers || []).forEach((bezier) => {
		...
		if (points.length >= 4) {
			parts.push(
				`<path class="preview-line" d="M ${tx(points[0].x)} ${ty(points[0].y)} C ${tx(points[1].x)} ${ty(points[1].y)}, ${tx(points[2].x)} ${ty(points[2].y)}, ${tx(points[3].x)} ${ty(points[3].y)}"/>`,
			);
		}
	});
	(preview.pins || []).forEach((pin) => {
		...
		parts.push(
			`<line class="preview-pin" data-ref-kind="pin" data-ref-id="${pin.record_index}" data-designator="${svgEscape(pin.designator)}" data-pin="${svgEscape(pin.pin)}" x1="${tx(pin.x)}" y1="${ty(pin.y)}" x2="${tx(body.x)}" y2="${ty(body.y)}"><title>${tooltip}</title></line>`,
		);
		parts.push(
			`<circle class="preview-pin-hotspot" data-ref-kind="pin" data-ref-id="${pin.record_index}" data-designator="${svgEscape(pin.designator)}" data-pin="${svgEscape(pin.pin)}" cx="${tx(pin.x)}" cy="${ty(pin.y)}" r="1.8"><title>${tooltip}</title></circle>`,
		);
		const label = String(pin.pinName || pin.pin || "").trim();
		if (label) {
			const placement = previewPinLabelPlacement(pin, tx, ty);
			parts.push(
				`<text class="preview-text preview-pin-label" data-ref-kind="pin-label" data-ref-id="${pin.record_index}" x="${placement.x}" y="${placement.y}" text-anchor="${placement.anchor}" dominant-baseline="${placement.baseline}">${svgEscape(label)}</text>`,
			);
		}
	});
	(preview.texts || []).forEach((text) => {
		...
		const cssClass =
			text.kind === "designator"
				? "preview-text preview-text-designator"
				: text.kind === "parameter"
					? "preview-text preview-text-parameter"
					: "preview-text";
		parts.push(
			`<text class="${cssClass}" x="${x}" y="${y}"${transform}>${svgEscape(text.text)}</text>`,
		);
	});
	(preview.junctions || []).forEach((point) => {
		parts.push(
			`<circle class="preview-junction" cx="${tx(point.x)}" cy="${ty(point.y)}" r="2.5"/>`,
		);
	});
	(preview.noErcs || []).forEach((point) => {
		...
		const size = 4;
		parts.push(
			`<g class="preview-noerc"><line x1="${x - size}" y1="${y - size}" x2="${x + size}" y2="${y + size}"/><line x1="${x - size}" y1="${y + size}" x2="${x + size}" y2="${y - size}"/></g>`,
		);
	});
	(preview.netLabels || []).forEach((label) => {
		...
		parts.push(
			`<g${transform}><text class="preview-text preview-netlabel" data-ref-kind="netlabel" x="${placement.x}" y="${placement.y}" text-anchor="${anchor}" dominant-baseline="${baseline}">${svgEscape(label.text)}</text></g>`,
		);
	});
	(preview.powerPorts || []).forEach((port) => {
		const placement = previewPowerPortPlacement(port, tx, ty);
		parts.push(
			`<text class="preview-text preview-powerport" data-ref-kind="power" x="${placement.x}" y="${placement.y}" text-anchor="${placement.anchor}" dominant-baseline="${placement.baseline}">${svgEscape(port.text)}</text>`,
		);
	});
	(preview.ports || []).forEach((port) => {
		const x = tx(port.x);
		const y = ty(port.y);
		const w = port.width || 46;
		const h = port.height || 10;
		const alignment = port.alignment || 1;
		const flip = alignment === 1 ? -1 : 1;
		parts.push(
			`<rect class="preview-box" x="${x - w / 2}" y="${y - h / 2}" width="${w}" height="${h}" rx="1" ry="1" data-ref-kind="port" data-ref-text="${svgEscape(port.text || "")}"/>`,
		);
		if (port.text) {
			parts.push(
				`<text class="preview-text preview-netlabel" data-ref-kind="port-label" x="${x + flip * (w / 2 + 3)}" y="${y}" text-anchor="${alignment === 1 ? "start" : "end"}" dominant-baseline="central">${svgEscape(port.text)}</text>`,
			);
		}
	});
	parts.push("</svg>");
...
function buildVisualSummary(records, partMap) {
	...
	return {
		...
		bounds: collectPointBounds(
			devices.flatMap((device) =>
				Array.isArray(device.coords) ? device.coords : [],
			),
		),
	};
}
...
function parseSchDocBuffer(fileBuffer) {
	...
	const nets = propagateNetNamesThroughPassives(
		buildNets(parsed.records, partMap),
		partMap,
	);
