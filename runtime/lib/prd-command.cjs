...
const PRD_EXPLORATION_PROMPTS = [
	"先用开放式问题探索用户想要的产品行为、使用场景和最小可验证交付，不要只根据硬件事实代填 PRD。",
	"逐项确认输入/输出、默认状态、异常/掉电/低压/复位行为、记忆策略和验收证据。",
	"把原理图/手册推断与用户确认分开：尤其 MCU 封装脚号到 GPIO/外设/网络的映射必须有来源或人工确认。",
	"从系统 PRD 拆出至少一个可执行子 PRD（docs/prd/features|modules|components|subsystems/*.md），再确认并生成任务。",
	"把确认后的结构化目标、功能、约束、验收和未知项同步到 .emb-agent/req.yaml，不能保留模板占位。",
];
const REQ_TEMPLATE_MARKERS = [
	"{{",
	"Define the first deliverable target for the current project",
	"Complete the most critical board-level behavior or feature closure",
	"The current goal can be confirmed at board level or through a minimal verification path",
	"Customer or production requirements still need confirmation",
];
const EXECUTION_PRD_LOCATIONS = [
	"docs/prd/features/<feature>.md",
	"docs/prd/modules/<module>.md",
	"docs/prd/components/<component>.md",
	"docs/prd/subsystems/<subsystem>.md",
	"docs/prd/verification.md",
];
const IGNORED_PRD_DIRS = new Set(["archive", "tasks"]);
const EXECUTION_PRD_DIRS = new Set([
	"subsystems",
	"features",
	"modules",
	"components",
]);
const VERIFICATION_FILE_NAMES = new Set([
	"verification",
	"validation",
	"test-plan",
	"acceptance",
...
function getConfirmationPath(projectRoot, deps) {
	...
	return path.join(
		getProjectExtDir(runtime, projectRoot),
...
function classifyPrdPath(relativePath) {
	...
	const fileName = (afterRoot[afterRoot.length - 1] || "").replace(
		/\.md$/i,
		"",
	);
	...
	if (
		first === "system.md" ||
		(afterRoot.length === 1 && fileName === "system")
	) {
		return "system";
	}
...
function shouldIgnorePrdPath(relativePath) {
	...
	const parts = normalized.split("/");
	const afterRoot =
		parts[0] === "docs" && parts[1] === "prd" ? parts.slice(2) : parts;
...
function buildTaskName(doc, usedNames) {
	...
	const fileBase = (pathParts[pathParts.length - 1] || "prd-task").replace(
		/\.md$/i,
		"",
	);
	...
	const baseSlug = slugify(
		fileBase,
		slugify(cleanPrdTitle(doc.title), "prd-task"),
	);
...
function isExecutionPrd(doc) {
	return Boolean(
		doc && (doc.kind === "execution" || doc.kind === "verification"),
	);
}
...
function buildPrdTaskPlan(projectRoot, deps) {
	...
	return documents.filter(isExecutionPrd).map((doc) => {
		const name = buildTaskName(doc, usedNames);
		const exists = fs.existsSync(
			path.join(existingTasksDir, name, "task.json"),
		);
		return {
			source: doc.path,
			kind: doc.kind,
...
function compareManifest(left, right) {
	...
	const byPath = new Map(
		leftDocs.map((item) => [normalizePath(item.path), item.sha256]),
	);
	return rightDocs.every(
		(item) => byPath.get(normalizePath(item.path)) === item.sha256,
	);
}

function getReqTemplateMarkers(text) {
	const content = String(text || "");
	const markers = REQ_TEMPLATE_MARKERS.filter((marker) =>
		content.includes(marker),
	);
	if (
		/goals:\s*\r?\n\s*-\s*["']?Define the first deliverable target for the current project["']?/i.test(
			content,
		)
	) {
		markers.push("default goals");
	}
	return runtimeUnique(markers);
}

function summarizeRequirementTruth(projectRoot, deps) {
	const { fs, path, runtime } = deps;
	const reqRelativePath = runtime.getProjectAssetRelativePath("req.yaml");
	const reqPath = path.join(getProjectExtDir(runtime, projectRoot), "req.yaml");
	const exists = fs.existsSync(reqPath);
	const content = readTextIfExists(fs, reqPath);
	const templateMarkers = getReqTemplateMarkers(content);
	const templateLike = !exists || templateMarkers.length > 0;

	return {
		path: reqRelativePath,
		exists,
		status: exists ? (templateLike ? "template" : "ready") : "missing",
		template_like: templateLike,
		template_markers: templateMarkers,
		summary: exists
			? templateLike
				? `${reqRelativePath} still looks like the initialization template; mirror confirmed PRD facts before confirmation.`
				: `${reqRelativePath} contains non-template requirement truth.`
			: `${reqRelativePath} is missing.`,
		recommendation: exists
			? templateLike
				? `Update ${reqRelativePath} with confirmed goals, features, constraints, acceptance, unknowns, and sources from the PRD discussion.`
				: ""
			: `Create ${reqRelativePath} before confirming PRD.`,
	};
}

function buildPrdExplorationState(projectRoot, deps, input = {}) {
	const documents = Array.isArray(input.documents)
		? input.documents
		: listPrdDocuments(projectRoot, deps);
	const taskPlan = Array.isArray(input.task_plan)
		? input.task_plan
		: buildPrdTaskPlan(projectRoot, deps);
	const requirementTruth =
		input.requirement_truth || summarizeRequirementTruth(projectRoot, deps);
	const hasSystemPrd = documents.some((item) => item && item.kind === "system");
	const plannedTaskCount = taskPlan.filter(
		(item) => item && item.status === "planned",
	).length;
	const hasExecutionPrd = taskPlan.length > 0;
	const missingExecutionPrds = hasSystemPrd && !hasExecutionPrd;
	const requiredUpdates = runtimeUnique([
		requirementTruth.status !== "ready"
			? `Mirror confirmed goals/features/constraints/acceptance/unknowns into ${requirementTruth.path}; do not leave template placeholders.`
			: "",
		missingExecutionPrds
			? `Create at least one child execution PRD under ${EXECUTION_PRD_LOCATIONS.slice(0, 4).join(" or ")} before running prd confirm --create-tasks.`
			: "",
		"Ask requirement exploration questions first; record uncertain answers as unknowns instead of assumptions.",
		"Review schematic-derived pin roles with datasheet/manual evidence or explicit user confirmation before promoting them to hw.yaml/system PRD.",
	]);
	const blocksConfirmation =
		requirementTruth.status !== "ready" || missingExecutionPrds;

	return {
		status: blocksConfirmation
			? "needs-requirement-exploration"
			: "ready-for-prd-confirmation",
		blocking: blocksConfirmation,
		blocks_confirmation: blocksConfirmation,
		system_prd_path:
			documents.find((item) => item && item.kind === "system")?.path ||
			pathJoinNormalized(PRD_ROOT, "system.md"),
		requirement_truth: requirementTruth,
		missing_execution_prds: missingExecutionPrds,
		has_execution_prds: hasExecutionPrd,
		planned_task_count: plannedTaskCount,
		child_prd_locations: EXECUTION_PRD_LOCATIONS.slice(),
		prompts: PRD_EXPLORATION_PROMPTS.slice(),
		required_updates: requiredUpdates,
		summary: blocksConfirmation
			? "System PRD is not ready for confirmation: requirements must be explored, req.yaml must be non-template, and child execution PRDs must exist before task creation."
			: "System PRD, requirement truth, and child execution PRDs are ready for PRD confirmation.",
	};
}

function pathJoinNormalized(...parts) {
	return normalizePath(parts.filter(Boolean).join("/"));
}

function runtimeUnique(values) {
	return [
		...new Set(
...
function summarizePrdConfirmationState(projectRoot, deps) {
	...
	const confirmation = fs.existsSync(confirmationPath)
		? readJson(fs, confirmationPath)
		: null;
	...
	const stale = Boolean(
		confirmation && confirmation.status === "confirmed" && !confirmed,
	);
	const taskPlan = buildPrdTaskPlan(projectRoot, deps);
	const plannedTasks = taskPlan.filter((item) => item.status === "planned");
	const requirementTruth = summarizeRequirementTruth(projectRoot, deps);
	const exploration = buildPrdExplorationState(projectRoot, deps, {
		documents,
		task_plan: taskPlan,
		requirement_truth: requirementTruth,
	});
	...
	return {
		...
		confirmed,
		stale,
		confirmation_path: normalizePath(
			deps.path.relative(projectRoot, confirmationPath),
		),
		confirmed_at:
			confirmation && confirmation.confirmed_at
...
function createExecutionTasks(projectRoot, deps, taskPlan, documents) {
	...
	const docTitleByPath = new Map(
		documents.map((item) => [item.path, item.title]),
	);
	const existingVerification = documents.find(
	...
	for (const task of taskPlan) {
		...
		const taskPrd = normalizePath(
			path.join(PRD_ROOT, "tasks", `${task.name}.md`),
		);
		const references = [
			documents.some((item) => item.path === "docs/prd/system.md")
				? "docs/prd/system.md"
				: "",
			task.source,
			existingVerification && existingVerification.path !== task.source
				? existingVerification.path
				: "",
			fs.existsSync(path.join(projectRoot, ".emb-agent", "hw.yaml"))
				? ".emb-agent/hw.yaml"
				: "",
			fs.existsSync(path.join(projectRoot, ".emb-agent", "req.yaml"))
				? ".emb-agent/req.yaml"
				: "",
...
function buildPrdTaskAlignment(taskCreation) {
	...
	return {
		...
		summary:
			"After PRD confirmation creates execution tasks, align the generated task split, unclear requirements, boundaries, and acceptance evidence with the user before activation or implementation.",
		...
		next_after_agreement: first.name
			? {
					...
					reason:
						"User has explicitly agreed that the generated task split and first task PRD are clear enough to proceed.",
				}
			: null,
	};
}
...
function createPrdCommandHelpers(deps) {
	const { fs, path, runtime, resolveProjectRoot, updateSession } = deps;
	...
	function handlePrdCommands(cmd, subcmd, rest) {
		...
		if (
			!fs.existsSync(
				runtime.resolveProjectDataPath(projectRoot, "project.json"),
			)
		) {
		...
		if (subcmd === "confirm") {
			...
			const state = summarizePrdConfirmationState(projectRoot, {
				fs,
				path,
				runtime,
			});
			if (state.exploration && state.exploration.blocks_confirmation) {
				updateSession((current) => {
					current.last_command = "prd confirm";
				});
				return {
					command: "prd confirm",
					status: "blocked-by-prd-exploration",
					summary: state.exploration.summary,
					confirmation_path: state.confirmation_path,
					documents: state.documents,
					task_plan: state.task_plan,
					planned_task_count: state.planned_task_count,
					requirement_truth: state.requirement_truth,
					exploration: state.exploration,
					next: {
						command: "ai-host explore-prd",
						reason: state.exploration.summary,
						cli: "",
					},
				};
			}
			...
			const confirmationPath = getConfirmationPath(projectRoot, {
				fs,
				path,
				runtime,
			});
			...
			return {
				...
				confirmation_path: normalizePath(
					path.relative(projectRoot, confirmationPath),
				),
				documents: state.documents,
				...
				next:
					taskCreation.created.length > 0
						...
						: {
								...
								reason:
									"PRD is confirmed. Create a concrete execution task when ready.",
		...
		if (subcmd === "help" || subcmd === "--help" || subcmd === "-h") {
			return {
				command: "prd",
				usage: ["prd status", "prd confirm [--create-tasks|--no-create-tasks]"],
				notes: [
...
module.exports = {
	PRD_CONFIRMATION_FILE,
	PRD_EXPLORATION_PROMPTS,
	summarizeRequirementTruth,
	buildPrdExplorationState,
