...
test('applyOutputMode builds brief next context payload', () => {
  ...
  assert.equal(output.agent_protocol.audience, 'ai-host');
  assert.equal(output.agent_protocol.recommendation.command, 'plan');
  assert.equal(output.agent_protocol.recommendation.cli, 'node runtime/bin/emb-agent.cjs capability run plan');
  assert.match(output.agent_protocol.recommendation.reason, /complex tasks should converge first/);
  assert.match(output.agent_protocol.ai_instruction.raw_output_policy, /Machine output is for AI routing only/);
...
test('applyOutputMode builds brief start context payload with external driver hints', () => {
  ...
  assert.equal(output.agent_protocol.gate.kind, 'task-intake');
  assert.equal(output.agent_protocol.gate.blocking, true);
  assert.ok(output.agent_protocol.gate.allowed_actions.includes('task add <summary>'));
  assert.match(output.agent_protocol.ai_instruction.ask_user, /具体任务|concrete task/i);
...
test('applyOutputMode hides internal trust details in brief health/bootstrap outputs', () => {
  ...
  assert.equal('action_card' in healthOutput, false);
  assert.equal(healthOutput.agent_protocol.gate.kind, 'health');
  assert.equal(healthOutput.agent_protocol.gate.blocking, true);
  assert.match(healthOutput.agent_protocol.ai_instruction.ask_user, /健康检查|health/i);
  ...
  assert.equal('action_card' in bootstrapOutput, false);
  assert.equal(bootstrapOutput.agent_protocol.gate.kind, 'health');
...
test('applyOutputMode builds brief bootstrap output', () => {
  ...
  assert.equal(output.agent_protocol.gate.kind, 'health');
  assert.equal(output.agent_protocol.gate.blocking, true);
