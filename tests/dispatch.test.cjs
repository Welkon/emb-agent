...
const dispatchCommandRuntimeHelpers = require(
	path.join(repoRoot, "runtime", "lib", "dispatch-command-runtime.cjs"),
);
...
test("dispatch show returns direct action contract for plan", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-plan-"),
	);
	...
	try {
		...
		assert.equal(
			dispatch.agent_execution.dispatch_contract.primary.agent,
			"emb-hw-scout",
		);
		assert.equal(
			dispatch.agent_execution.dispatch_contract.primary.spawn_fallback
				.fallback_agent_type,
			"explorer",
		);
		assert.ok(
			dispatch.agent_execution.dispatch_contract.primary.worker_contract,
		);
		assert.ok(
			dispatch.agent_execution.dispatch_contract.primary.worker_contract.inputs
				.length > 0,
		);
		assert.ok(
			dispatch.agent_execution.dispatch_contract.primary.worker_contract
...
test("normalized session seeds empty delegation runtime diagnostics", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-session-seed-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		assert.deepEqual(
			session.diagnostics.delegation_runtime.launch_requests,
			[],
		);
		assert.equal(
			session.diagnostics.delegation_runtime.synthesis.required,
			false,
		);
		assert.equal(
			session.diagnostics.delegation_runtime.integration.execution_kind,
			"",
		);
	} finally {
		process.chdir(currentCwd);
...
test("dispatch next follows PRD exploration gate before task/debug routing", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-next-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		assert.equal(dispatch.resolved_action, "ai-host explore-prd");
		assert.equal(dispatch.cli, "");
		assert.equal(dispatch.workflow_stage.name, "prd-exploration");
		assert.equal(
			dispatch.workflow_stage.primary_command,
			"ai-host explore-prd",
		);
		assert.equal(dispatch.knowledge_graph.state, "missing");
		assert.ok(
			dispatch.knowledge_graph.next_steps.includes("knowledge graph refresh"),
		);
		assert.equal(dispatch.agent_execution.inline_ok, true);
		assert.equal(dispatch.agent_execution.recommended, false);
		assert.ok(dispatch.reason.includes("System PRD is not ready"));
	} finally {
		process.chdir(currentCwd);
		process.stdout.write = originalWrite;
	}
...
test("dispatch next returns arch-review contract when focus triggers architecture review", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-arch-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		cli.main([
			"focus",
			"set",
			"chip selection and PoC to production preflight",
		]);
		...
		assert.equal(
			dispatch.agent_execution.dispatch_contract.auto_invoke_when_recommended,
			true,
		);
		assert.equal(
			dispatch.agent_execution.dispatch_contract.delegation_pattern,
			"coordinator",
		);
		assert.equal(
			dispatch.agent_execution.dispatch_contract.pattern_constraints.max_depth,
			1,
		);
		assert.equal(
			dispatch.agent_execution.dispatch_contract.synthesis_required,
			true,
		);
		assert.equal(
			dispatch.agent_execution.dispatch_contract.primary.spawn_fallback
		...
		assert.ok(
			dispatch.agent_execution.dispatch_contract.primary.worker_contract.outputs
				.length > 0,
		);
		assert.ok(
			dispatch.agent_execution.dispatch_contract.supporting.every(
				(item) => item.worker_contract,
			),
		);
	} finally {
		process.chdir(currentCwd);
...
test("dispatch next routes hardware formula questions to scan before debug", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-scan-tool-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		cli.main([
			"question",
			"add",
			"how should tm2 prescaler and pwm formulas be calculated",
		]);
...
test("dispatch next exposes direct tool execution when scan has ready recommendation", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-tool-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		fs.mkdirSync(path.join(projectEmbDir, "extensions", "chips", "profiles"), {
			recursive: true,
		});
		fs.mkdirSync(path.join(projectEmbDir, "extensions", "tools", "families"), {
			recursive: true,
		});
		fs.mkdirSync(path.join(projectEmbDir, "extensions", "tools", "devices"), {
			recursive: true,
		});
		fs.writeFileSync(
		...
		assert.deepEqual(dispatch.tool_execution.argv.slice(0, 3), [
			"tool",
			"run",
			"timer-calc",
		]);
		assert.equal(
			dispatch.action_context.recommended_sources[0].id,
			"mcu/vendor-chip-registers",
		);
		assert.match(dispatch.tool_execution.cli, /tool run timer-calc/);
		assert.deepEqual(dispatch.tool_execution.missing_inputs, [
			"clock-hz",
			"target-us or target-hz",
		]);
	} finally {
		process.chdir(currentCwd);
		process.stdout.write = originalWrite;
...
test("dispatch run returns a structured draft when recommended tool still needs inputs", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-run-tool-"),
	);
	const currentCwd = process.cwd();
	const projectEmbDir = path.join(tempProject, ".emb-agent");
	...
	try {
		...
		assert.deepEqual(run.missing_inputs, [
			"clock-hz",
			"target-us or target-hz",
		]);
...
test("dispatch run returns missing tool inputs instead of throwing when tool-first routing lacks parameters", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-run-tool-missing-"),
	);
	const currentCwd = process.cwd();
	const projectEmbDir = path.join(tempProject, ".emb-agent");
...
test("dispatch run advances peripheral walkthrough step-by-step and persists progress", () => {
	...
	function buildDispatchContext() {
		...
		return {
			...
			walkthrough_execution: {
				...
				completed_count: Number.isInteger(runtimeState.completed_count)
					? runtimeState.completed_count
					: 0,
				total_steps: orderedTools.length,
				current_tool:
					currentStep && currentStep.tool ? currentStep.tool : orderedTools[0],
			},
			tool_execution: {
				available: true,
				recommended: true,
	...
	const firstRun = helpers.executeDispatchCommand("next", {
		entered_via: "dispatch run next",
	});
	...
	const secondRun = helpers.executeDispatchCommand("next", {
		entered_via: "dispatch run next",
	});
...
test("dispatch run persists delegation runtime snapshot into session diagnostics", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-runtime-"),
	);
	const currentCwd = process.cwd();
	...
	try {
		...
		assert.ok(
			delegationRuntime.launch_requests.some(
				(item) => item.agent === "emb-hw-scout",
			),
		);
		...
		assert.equal(
			delegationRuntime.review.stage_a.status,
		...
		assert.equal(
			delegationRuntime.integration.entered_via,
			"dispatch run next",
		);
	} finally {
		process.chdir(currentCwd);
...
test("dispatch run invokes configured host sub-agent bridge and stores worker results", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-worker-"),
	);
	const currentCwd = process.cwd();
...
test("dispatch launch registers async delegation jobs and collect persists completed worker results", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-launch-"),
	);
	const currentCwd = process.cwd();
	const originalBridgeCmd = process.env.EMB_AGENT_SUBAGENT_BRIDGE_CMD;
	...
	try {
		...
		assert.equal(
			launchSession.diagnostics.delegation_runtime.jobs[0].status,
			"running",
		);
		...
		assert.equal(
			collectedSession.diagnostics.delegation_runtime.worker_results.length,
			1,
		);
		assert.equal(
			collectedSession.diagnostics.delegation_runtime.jobs[0].status,
			"completed",
		);
	} finally {
		if (originalBridgeCmd === undefined) {
...
test("dispatch launch falls back to manual worker requests without host bridge", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-launch-manual-"),
	);
	const currentCwd = process.cwd();
	const originalBridgeCmd = process.env.EMB_AGENT_SUBAGENT_BRIDGE_CMD;
	...
	try {
		...
		assert.ok(
			launch.delegation_runtime.launch_requests.some(
				(item) => item.agent === "emb-hw-scout",
			),
		);
		assert.equal(
			launch.delegation_runtime.synthesis.status,
			"manual-workers-required",
		);
		assert.equal(
			launch.delegation_runtime.review.stage_a.status,
			"pending-worker-results",
		);
	} finally {
		if (originalBridgeCmd === undefined) {
...
test("dispatch run hard-blocks action execution when stage A review requires redispatch", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-redispatch-"),
	);
	const currentCwd = process.cwd();
	const originalBridgeCmd = process.env.EMB_AGENT_SUBAGENT_BRIDGE_CMD;
	...
	try {
		...
		assert.equal(
			run.delegation_runtime.review.stage_a.status,
			"redispatch-required",
		);
		assert.equal(run.redispatch_required, true);
		assert.equal(
			run.summary.includes("Stage A status=redispatch-required"),
			true,
		);
	} finally {
		if (originalBridgeCmd === undefined) {
...
test("dispatch next keeps current context focused on session carry-over", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-session-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
...
test("dispatch next routes drift-style failures to review", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-review-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
...
test("dispatch next exposes structured executor signal when latest executor failed", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-dispatch-executor-signal-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
