...
test('orchestrator keeps tool step non-required when adapter trust is not yet executable', () => {
  ...
  try {
    ...
    assert.ok(next.next_actions.some(item => item.includes('chip_support_status=reusable candidate after review')));
    assert.ok(next.next_actions.some(item => item.includes('chip_support_trust=')));
