...
test('manager view surfaces tool execution before generic next action when ready', () => {
  ...
  try {
    ...
    assert.equal(manager.tool_execution.trust.grade, 'usable');
    assert.equal(manager.tool_execution.trust.executable, true);
    assert.equal(manager.health.adapter_health.primary.tool, 'timer-calc');
    assert.equal(manager.health.adapter_health.primary.grade, 'usable');
    const toolIndex = manager.recommended_actions.findIndex(item => item.type === 'tool');
    const nextIndex = manager.recommended_actions.findIndex(item => item.type === 'next');
    const timerCalcActions = manager.recommended_actions.filter(item => item.cli.includes('tool run timer-calc'));
    assert.ok(toolIndex >= 0);
    assert.ok(nextIndex >= 0);
    assert.ok(toolIndex < nextIndex);
    assert.equal(timerCalcActions.length, 1);
  } finally {
    process.chdir(currentCwd);
    process.stdout.write = originalWrite;
  }
...
test('manager prioritizes adapter-health action before next when primary tool is not yet trustworthy', () => {
  const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), 'emb-agent-manager-adapter-health-'));
  const currentCwd = process.cwd();
  const originalWrite = process.stdout.write;
  const projectEmbDir = path.join(tempProject, '.emb-agent');
  process.stdout.write = () => true;
  ...
  try {
    ...
    assert.equal(manager.health.adapter_health.primary.tool, 'timer-calc');
    assert.equal(manager.health.adapter_health.primary.executable, false);
    assert.equal(manager.health.adapter_health.primary.recommended_action, 'implement-adapter');
    const adapterHealthIndex = manager.recommended_actions.findIndex(item => item.type === 'adapter-health');
    const nextIndex = manager.recommended_actions.findIndex(item => item.type === 'next');
    const timerCalcActions = manager.recommended_actions.filter(item => item.cli.includes('tool run timer-calc'));
    assert.ok(adapterHealthIndex >= 0);
    assert.ok(nextIndex >= 0);
    assert.ok(adapterHealthIndex < nextIndex);
    assert.equal(timerCalcActions.length, 1);
    assert.equal(manager.recommended_actions[adapterHealthIndex].reason.includes('implement-adapter'), true);
  } finally {
    process.chdir(currentCwd);
    process.stdout.write = originalWrite;
  }
