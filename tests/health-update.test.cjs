...
function createHealthAdapterSource(rootDir) {
	...
	writeJson(
		path.join(rootDir, "extensions", "tools", "families", "scmcu-sc8f0xx.json"),
		{
			name: "scmcu-sc8f0xx",
			vendor: "SCMCU",
			series: "SC8F0xx",
	...
	writeJson(
		path.join(rootDir, "extensions", "tools", "devices", "sc8f072.json"),
		{
			name: "sc8f072",
	...
	writeJson(
		path.join(rootDir, "extensions", "chips", "profiles", "sc8f072.json"),
		{
			name: "sc8f072",
			vendor: "SCMCU",
			family: "scmcu-sc8f0xx",
...
test("health reports warn for incomplete hardware identity and fail for missing truth files", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-"),
	);
	...
	try {
		...
		assert.ok(
			report.checks.some(
				(item) => item.key === "project_config_valid" && item.status === "pass",
			),
		);
		assert.ok(
			report.checks.some(
				(item) => item.key === "subagent_bridge" && item.status === "pass",
			),
		);
		assert.equal(report.subagent_bridge.mode, "mock");
		assert.ok(
			report.checks.some(
				(item) => item.key === "hardware_identity" && item.status === "warn",
			),
		);
		assert.ok(!report.checks.some((item) => item.key === "szlcsc_integration"));
		assert.ok(Array.isArray(report.next_commands));
		assert.ok(
			report.next_commands.every(
				(item) => !item.cli.includes("support source add default-support"),
			),
		);
		...
		assert.match(
			report.quickstart.user_summary,
		...
		assert.match(
			report.action_card.first_instruction,
			/Hardware identity is incomplete/,
		);
		...
		assert.equal(
			report.bootstrap.next_stage.display_status,
			"needs-user-input",
		);
		assert.equal(report.bootstrap.next_stage.display_id, "project-facts");
		assert.equal(
			report.bootstrap.next_stage.action_summary,
			"Project facts required",
		);
		assert.ok(
			report.bootstrap.stages.some(
				(item) => item.id === "init-project" && item.status === "completed",
			),
		);
		...
		stdout = "";
		fs.rmSync(path.join(tempProject, ".emb-agent", "req.yaml"), {
			force: true,
		});
		cli.main(["health"]);
		...
		assert.ok(
			report.checks.some(
				(item) => item.key === "req_truth" && item.status === "fail",
			),
		);
...
test("health specs reports active code-writing spec enforcement", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-specs-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	let stdout = "";
	...
	try {
		...
		assert.ok(
			report.checks.some(
				(item) =>
					item.key === "code_writing_specs_enforced" && item.status === "pass",
			),
		);
		assert.deepEqual(report.specs_doctor.active_specs, ["embedded-space"]);
		assert.equal(
			report.specs_doctor.selected_code_writing_specs[0].name,
			"embedded-space",
		);
		assert.equal(
			report.specs_doctor.selected_code_writing_specs[0].path,
			".emb-agent/specs/embedded-space.md",
		);
		...
		assert.equal(
			brief.specs_doctor.selected_code_writing_specs[0].name,
			"embedded-space",
		);
		assert.equal("human_reply" in brief, false);
		assert.equal(brief.agent_protocol.gate.kind, "health");
		assert.match(
			brief.agent_protocol.ai_instruction.raw_output_policy,
...
test("health prioritizes discovered schematic intake before manual hardware identity", async (t) => {
	...
	try {
		...
		fs.copyFileSync(
			fixturePath,
			path.join(tempProject, "docs", "board.SchDoc"),
		);
		...
		assert.equal(
			report.checks.find((item) => item.key === "project_source_intake").status,
			"warn",
		);
		assert.equal(report.bootstrap.current_stage, "source-intake");
		assert.equal(
			report.bootstrap.display_current_stage,
			"normalize-discovered-inputs",
		);
		assert.equal(report.quickstart.stage, "ingest-detected-input");
		assert.equal(
			report.quickstart.display_stage,
			"normalize-discovered-inputs",
		);
		assert.ok(
			report.bootstrap.next_stage.cli.includes(
				"ingest schematic --file docs/board.SchDoc",
			),
		);
...
test("health reports fallback session state path when primary state storage is readonly", {
	concurrency: false,
}, () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-fallback-"),
	);
	const fallbackStateDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-fallback-state-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	const statePaths = runtime.getProjectStatePaths(
		path.join(repoRoot, "runtime"),
		tempProject,
		runtimeConfig,
	);
	...
	function isReadonlyPrimaryPath(filePath) {
		...
		return (
			normalized === path.resolve(statePaths.primaryStateDir) ||
			normalized.startsWith(readonlyPrefix)
		);
	}
	...
	try {
		...
		process.env.EMB_AGENT_WORKSPACE_TRUST = "1";
		const resolvedStatePaths = runtime.getProjectStatePaths(
			path.join(repoRoot, "runtime"),
			tempProject,
			runtimeConfig,
		);
		...
		assert.ok(
			sessionCheck.evidence.includes(resolvedStatePaths.fallbackSessionPath),
		);
...
test("health uses configured default adapter source from environment", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-private-source-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const originalBridgeCmd = process.env.EMB_AGENT_SUBAGENT_BRIDGE_CMD;
	...
	const previousLocation =
		process.env.EMB_AGENT_DEFAULT_CHIP_SUPPORT_SOURCE_LOCATION;
	const previousBranch =
		process.env.EMB_AGENT_DEFAULT_CHIP_SUPPORT_SOURCE_BRANCH;
	const previousSubdir =
		process.env.EMB_AGENT_DEFAULT_CHIP_SUPPORT_SOURCE_SUBDIR;
	let stdout = "";
...
test("health and bootstrap expose host startup readiness as an explicit bootstrap boundary", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-trust-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		assert.equal(
			report.checks.find((item) => item.key === "startup_automation").status,
			"warn",
		);
		...
		assert.ok(
			report.recommendations.some((item) => item.includes("automatic startup")),
		);
		...
		assert.ok(
			report.stages.some(
				(item) => item.id === "startup-hooks" && item.status === "manual",
			),
		);
	} finally {
...
test("installed codex runtime uses enabled hooks config as authorization signal", async () => {
	const tempHome = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-hooks-home-"),
	);
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-hooks-project-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const previousWorkspaceTrust = process.env.EMB_AGENT_WORKSPACE_TRUST;
	...
	try {
		...
		const installedCli = require(
			path.join(tempHome, "emb-agent", "bin", "emb-agent.cjs"),
		);
		...
		assert.equal(report.quickstart.display_stage, "complete-project-facts");
		assert.ok(
			!report.bootstrap.stages.some((item) => item.id === "startup-hooks"),
		);
		assert.ok(
			!report.recommendations.some((item) =>
				item.includes("automatic startup"),
			),
		);
	} finally {
		if (previousWorkspaceTrust === undefined) {
...
test("bootstrap run bypasses startup-hooks but respects task-intake gate", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-bootstrap-run-bypass-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const previousWorkspaceTrust = process.env.EMB_AGENT_WORKSPACE_TRUST;
	...
	try {
		...
		await cli.main([
			...
			"path",
			"--location",
			path.resolve(repoRoot, "..", "emb-support"),
			"--subdir",
			"adapters",
		]);
		...
		assert.equal(report.result.status, "blocked-by-prd-exploration");
		assert.equal(report.result.resolved_action, "ai-host explore-prd");
	} finally {
		if (previousWorkspaceTrust === undefined) {
			delete process.env.EMB_AGENT_WORKSPACE_TRUST;
		} else {
...
test("update reports stale install and cached newer version", () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-update-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	...
	try {
		...
		assert.ok(
			report.recommendations.some((item) =>
				item.includes("hooks / runtime / agents"),
			),
		);
		assert.equal(
			report.workflow_layout.registry_path,
...
test("health reports adapter registration and sync readiness", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-adapter-"),
	);
	const tempSource = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-adapter-source-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const previousTrust = process.env.EMB_AGENT_WORKSPACE_TRUST;
	...
	try {
		...
		assert.equal(
			report.checks.find(
				(item) => item.key === "chip_support_sources_registered",
			).status,
			"info",
		);
		assert.equal(
			report.checks.find((item) => item.key === "chip_support_sync_project")
				.status,
			"info",
		);
		assert.ok(
			report.next_commands.some((item) =>
				item.cli.includes("adapter derive --from-project"),
			),
		);
		...
		assert.ok(
			report.action_card.first_cli.includes("adapter derive --from-project"),
		);
		assert.ok(report.action_card.then_cli.endsWith(" next"));
		assert.equal(report.bootstrap.current_stage, "support-derive");
		assert.ok(
			report.bootstrap.next_stage.cli.includes("adapter derive --from-project"),
		);
		assert.ok(
			report.quickstart.steps[0].cli.includes("adapter derive --from-project"),
		);
		assert.ok(report.quickstart.steps[1].cli.endsWith(" next"));

		stdout = "";
		await cli.main(["bootstrap"]);
		const bootstrapView = JSON.parse(stdout);
		...
		assert.equal(
			report.checks.find((item) => item.key === "chip_support_match").status,
			"pass",
		);
		...
		assert.ok(
			report.next_commands.every(
				(item) => !item.cli.includes("support bootstrap default-support"),
			),
		);
		assert.equal(report.quickstart.stage, "next");
		assert.equal(
			report.checks.find((item) => item.key === "chip_support_quality").status,
			"warn",
		);
		assert.equal(
			report.checks.find((item) => item.key === "binding_quality").status,
			"warn",
		);
		assert.equal(
			report.checks.find((item) => item.key === "chip_support_reusability")
				.status,
			"info",
		);
		assert.equal(
			report.checks.find((item) => item.key === "register_summary_available")
				.status,
			"warn",
		);
		...
		assert.ok(
			report.recommendations.every(
				(item) => !item.includes("support sync default-support"),
			),
		);
		assert.ok(
			report.next_commands.some((item) =>
				item.cli.includes("tool run timer-calc"),
			),
		);
	} finally {
		if (previousTrust === undefined) {
...
test("health surfaces pending doc apply as quickstart before generic next", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-doc-apply-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const previousTrust = process.env.EMB_AGENT_WORKSPACE_TRUST;
	...
	try {
		...
		fs.mkdirSync(path.join(tempProject, "docs"), { recursive: true });
		fs.writeFileSync(
			path.join(tempProject, "docs", "PMS150G.pdf"),
			"fake pdf content",
			"utf8",
		);
		...
		assert.equal(
			report.checks.find((item) => item.key === "doc_apply_backlog").status,
			"warn",
		);
...
test("health routes from applied hardware doc to adapter derive when synced adapters still miss the chip", async () => {
	const tempProject = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-derive-"),
	);
	const tempSource = fs.mkdtempSync(
		path.join(os.tmpdir(), "emb-agent-health-derive-source-"),
	);
	const currentCwd = process.cwd();
	const originalWrite = process.stdout.write;
	const previousTrust = process.env.EMB_AGENT_WORKSPACE_TRUST;
	...
	try {
		...
		await cli.runIngestCommand("apply", [
			"doc",
			ingested.doc_id,
			"--to",
			"hardware",
		...
		assert.equal(
			report.checks.find((item) => item.key === "chip_support_derive_candidate")
				.status,
			"warn",
		);
		assert.ok(
			report.next_commands.some((item) => item.key === "support-analysis-init"),
		);
		assert.ok(
			report.next_commands.some(
				(item) => item.key === "support-derive-from-analysis",
			),
		);
		assert.ok(
			report.next_commands.some((item) =>
		...
		assert.equal(
			report.handoff_protocol.protocol,
			"emb-agent.chip-support-analysis/1",
		);
		assert.equal(
			report.handoff_protocol.artifact_path,
			".emb-agent/analysis/pms150g.json",
		);
		assert.deepEqual(
			report.next_commands.find((item) => item.key === "support-analysis-init")
				.argv,
			["adapter", "analysis", "init", "--chip", "PMS150G", "--package", "SOP8"],
		);
		assert.deepEqual(
			report.next_commands.find(
				(item) => item.key === "support-derive-from-analysis",
			).argv,
			[
				"adapter",
		...
		assert.equal(report.action_card.first_instruction, "");
		assert.ok(
			report.action_card.first_cli.includes(
				"adapter analysis init --chip PMS150G --package SOP8",
			),
		);
		assert.ok(
			report.action_card.then_cli.includes(
				"adapter derive --from-analysis .emb-agent/analysis/pms150g.json",
			),
		);
		assert.equal(report.bootstrap.current_stage, "support-derive");
		assert.ok(
			report.quickstart.steps[0].cli.includes(
				"adapter analysis init --chip PMS150G --package SOP8",
			),
		);
		assert.ok(
			report.quickstart.steps[1].cli.includes(
				"adapter derive --from-analysis .emb-agent/analysis/pms150g.json",
			),
		);
		...
		assert.equal(bootstrapRun.executed, true);
		assert.equal(bootstrapRun.stage.id, "support-derive");
		assert.deepEqual(bootstrapRun.stage.argv, [
			"adapter",
			"analysis",
			"init",
			"--chip",
		...
		assert.equal(
			fs.existsSync(
				path.join(tempProject, ".emb-agent", "analysis", "pms150g.json"),
			),
			true,
		);
	} finally {
