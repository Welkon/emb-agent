...
pub fn build_chip_swap_plan(ext_dir: &Path, hw_yaml: &str, from: &str, to: &str) -> ChipSwapPlan {
    use super::project::HardwareTruth;
    ...
    if required_code_changes.is_empty() {
        required_code_changes.push("No code changes required for pin-compatible swap.".to_string());
    }
...
pub fn build_chip_swap_confirm_json(ext_dir: &Path, hw_yaml: &str, from: &str, to: &str) -> String {
    let plan = build_chip_swap_plan(ext_dir, hw_yaml, from, to);
