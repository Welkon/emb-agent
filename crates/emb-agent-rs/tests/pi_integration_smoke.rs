...
fn pi_extension_exposes_unified_tool_layer() {
    ...
    for expected in [
        ...
        "Main-session default is researcher first when SDK/toolchain/API evidence is missing",
        ...
        "the default dispatch is researcher first when SDK/toolchain/API evidence is missing",
        "knowledge_search, knowledge_diagnose, and knowledge_graph_query",
        "Use knowledge_search for project knowledge",
        "recordKnowledgePriming",
        "tool-guard-state.json",
        "knowledge_primed_at_ms",
        "pi-native-tool",
        "await hasFreshKnowledgePriming",
