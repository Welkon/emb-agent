...
If the dispatch prompt names `Target task: <name>`, read `.emb-agent/tasks/<name>/task.json`, then the PRD path listed in `task.json.artifacts.prd` (fallback: `.emb-agent/tasks/<name>/prd.md` when present) before reviewing system risk. If no target is named, keep the pass scoped to the explicit user request.
